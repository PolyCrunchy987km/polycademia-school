...
import { buildDataKeyboardFromStyle, convertClasseToString, } from '../lib/interactif/claviers/keyboard';
 ...
 */
export function mathalea2d({ xmin = 0, ymin = 0, xmax = 15, ymax = 6, pixelsParCm = 20, scale = 1, zoom = 1, optionsTikz = [], mainlevee = false, amplitude = 1, style = 'display: block', id = '', // L'id peut-être utile pour des animations, c'est celui du svg. Le div englobant aura un id en M2D + id
usePgfplots = false, centerLatex = false, } = {}, ...objets) {
    const ajouteCodeHtml = (mainlevee, objets, divsLatex, xmin, ymax) => {
        ...
        if (!Array.isArray(objets) && objets != null) {
            try {
                const objet = objets;
                if (!mainlevee || typeof (objet === null || objet === void 0 ? void 0 : objet.svgml) === 'undefined') {
                    if (objet === null || objet === void 0 ? void 0 : objet.svg) {
                        const code = objet.svg(pixelsParCm);
                        if (typeof code === 'string') {
                            codeSvg = '\t' + objet.svg(pixelsParCm) + '\n';
                        }
                        else {
                            const codeLatex = code;
                            // on a à faire à un divLatex.
                            if (typeof codeLatex !== 'object') {
                                window.notify("Dans mathalea2d, la méthode svg() de l'objet a renvoyé quelque chose d'inconnu", { codeLatex });
                                return codeSvg;
                            }
                            ...
                            if ('letterSize' in codeLatex) {
                                ...
                                codeLatex.color = codeLatex.color
                                    .replace('{', '')
                                    .replace('}', '');
                                const divOuterHtml = codeLatex.backgroundColor !== '' &&
                                ...
                                if ('inputs' in codeLatex) {
                                    ...
                                    for (const input of inputs) {
                                        ...
                                        const dataKeyboard = buildDataKeyboardString(convertClasseToString(input.classe));
                                ...
                                window.notify("Dans mathalea2d, la méthode svg() de l'objet a renvoyé un objet inconnu", { codeLatex });
                            }
                        }
                    }
                }
                else {
                    if (objet === null || objet === void 0 ? void 0 : objet.svgml) {
                        codeSvg = '\t' + objet.svgml(pixelsParCm, amplitude) + '\n';
                    }
                }
            }
            catch (error) {
                window.notify(error instanceof Error ? error.message : String(error), { objet: JSON.stringify(objets) });
            }
        }
        else {
    ...
    const ajouteCodeTikz = (mainlevee, objets, skipRepere = false, axisYMin, axisYMax, axisXMin, axisXMax) => {
        let codeTikz = '';
        ...
        if (skipRepere &&
            ...
            objets.isRepere === true) {
            return codeTikz;
        }
        // Don't expand Courbe objects when using pgfplots (they handle their own tikz output)
        // Check BEFORE expanding objets.objets
        const shouldNotExpand = objets instanceof ObjetMathalea2D && objets.usePgfplots === true;
        if (objets instanceof ObjetMathalea2D) {
        ...
        if (!Array.isArray(objets)) {
            ...
            for (const objet of objets) {
                codeTikz += ajouteCodeTikz(mainlevee, objet, skipRepere, axisYMin, axisYMax, axisXMin, axisXMax);
            }
        }
        return codeTikz;
    };
    ...
    const divsLatex = [];
    let codeSvg = `<svg class="mathalea2d"  ${id !== '' ? `id="${id}"` : ''} width="${(xmax - xmin) * pixelsParCm * zoom}" height="${(ymax - ymin) * pixelsParCm * zoom}" viewBox="${xmin * pixelsParCm} ${-ymax * pixelsParCm} ${(xmax - xmin) * pixelsParCm} ${(ymax - ymin) * pixelsParCm}" xmlns="http://www.w3.org/2000/svg" >\n`;
    ...
    if (style.includes('display: block') && !centerLatex)
        codeTikz += '\\\\\n';
    if (context.isHtml)
        return codeHTML;
    else
