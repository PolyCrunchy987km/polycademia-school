...
class EquationSecondDegre {
    constructor(a, b, c, d, e, f, options = { format: 'initial', variable: 'x' }) {
        const lang = getLang();
        this.coefficients = [a, b, c, d, e, f];
        this.fonctionEvaluer = (x) => (this.coefficients[0].num / this.coefficients[0].den) *
            Number(x) *
            Number(x) +
        ...
        if (options.format === 'reduit') {
            ...
            this.equationTex = this.printToLatexEq(tabMelange[0], tabMelange[1]);
        }
        ...
        this.delta = this.coefficientsEqReduite[1]
            ...
            .differenceFraction(this.coefficientsEqReduite[0]
        ...
        if (this.nombreSolutions > 0) {
            if (this.delta.estParfaite) {
                if (this.coefficientsEqReduite[1]
                    .multiplieEntier(-1)
                    .differenceFraction(new FractionEtendue(Math.sqrt(this.delta.num), Math.sqrt(this.delta.den)))
            ...
            if (this.natureDesSolutions === 'entier' ||
                this.natureDesSolutions === 'fractionnaire') {
                this.solutionsListeTex = [
                    `${this.coefficientsEqReduite[1]
                        .multiplieEntier(-1)
                        .differenceFraction(this.delta.racineCarree())
                        .produitFraction(this.coefficientsEqReduite[0].multiplieEntier(2).inverse()).texFractionSimplifiee}`,
                    `${this.coefficientsEqReduite[1]
                        .multiplieEntier(-1)
                        .sommeFraction(this.delta.racineCarree())
                        .produitFraction(this.coefficientsEqReduite[0].multiplieEntier(2).inverse()).texFractionSimplifiee}`,
                ];
                this.solutionsListeTexOppose = [
                    `${this.coefficientsEqReduite[1]
                        .multiplieEntier(-1)
                        .differenceFraction(this.delta.racineCarree())
                        .produitFraction(this.coefficientsEqReduite[0].multiplieEntier(2).inverse())
                ...
                if (this.delta.estEntiere) {
                    if (this.delta.num === 0) {
                        ...
                        let pgcdRacines = pgcd(2 *
                            this.coefficientsEqReduite[0].num *
                            this.coefficientsEqReduite[1].den, this.coefficientsEqReduite[1].num *
                            this.coefficientsEqReduite[0].den);
                        pgcdRacines = pgcd(pgcdRacines, this.coefficientsEqReduite[0].den *
                            this.coefficientsEqReduite[1].den *
                            racineDelta[0]);
                        // s = (a - b*sqrt(c))/d
                        const a = (-this.coefficientsEqReduite[1].num *
                            this.coefficientsEqReduite[0].den) /
                            pgcdRacines;
                        const b = (this.coefficientsEqReduite[0].den *
                            this.coefficientsEqReduite[1].den *
                            racineDelta[0]) /
                            pgcdRacines;
    ...
    static aPartirDuPolynome(polynome, options = { format: 'reduit', variable: 'x' }) {
        ...
        for (const monome of polynomeReduit.monomes) {
            ...
            const degree = varIndex !== -1 ? monome.partieLitterale.exposants[varIndex] : 0;
            if (degree === 2) {
        ...
        if (coeffX2.num === 0) {
            throw new Error("Le coefficient de x² est nul. Ce n'est pas une équation du second degré.");
        }
    ...
    complexifyCoefficients(coefficients) {
            ...
            if (this.solutionsListeTex[0].includes('sqrt')) {
                ...
                for (let j = 0; j < this.solutionsListeTex.length; j++) {
                    ...
                    if (!this.solutionsListeTex[j].includes('\\dfrac')) {
                        ...
                        numDen1 = [
                            Number((_a = this.solutionsListeTex[j]
                                .match(/\\dfrac{(\d+)}{(\d+)}/)) === null || _a === void 0 ? void 0 : _a.slice(1)[0]),
                            Number((_b = this.solutionsListeTex[j]
                                .match(/\\dfrac{(\d+)}{(\d+)}/)) === null || _b === void 0 ? void 0 : _b.slice(1)[1]),
                        ];
    ...
    printToLatex() {
        ...
        for (let i = 0; i < 3; i++) {
            if (this.coefficients.slice(0, 3).every((item) => item.num === 0) &&
                i === 0) {
                expr = expr + '0';
            }
        ...
        checkPreviousNull = true;
        for (let i = 3; i < 6; i++) {
            if (this.coefficients.slice(3).every((item) => item.num === 0) &&
                i === 3) {
                expr = expr + '0';
            }
            else if (!(this.coefficients[i].num === 0) && checkPreviousNull) {
    ...
    printToLatexMDG(option = { indice: -1, couleur: 'black' }) {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            if (i === option.indice) {
                if (suiteExpr.split(this.variable)[0] === '+' ||
                    suiteExpr.split(this.variable)[0] === '-') {
                    suiteExpr =
                        miseEnEvidence(suiteExpr.split(this.variable)[0] + '1', option.couleur) +
                            this.variable +
