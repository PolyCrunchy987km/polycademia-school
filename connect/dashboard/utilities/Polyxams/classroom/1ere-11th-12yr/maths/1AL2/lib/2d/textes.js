...
export function labelLatexPoint({ points, color, taille, largeur, hauteur, couleurDeRemplissage, } = {
    points: [],
    color: 'black',
    taille: 8,
    largeur: 10,
    ...
    listePoints = listePoints.map((pt) => 'c2d' in pt ? pt.c2d : pt);
    for (const unPoint of listePoints) {
        x = arrondi(unPoint.x);
        y = arrondi(unPoint.y);
        switch (unPoint.positionLabel) {
            case 'left':
                objets.push(latexParCoordonnees(unPoint.nom, x - offset, y, color, largeur, hauteur, couleurDeRemplissage, taille));
                break;
            case 'right':
                objets.push(latexParCoordonnees(unPoint.nom, x + offset, y, color, largeur, hauteur, couleurDeRemplissage, taille));
                break;
            case 'below':
                objets.push(latexParCoordonnees(unPoint.nom, x, y - offset, color, largeur, hauteur, couleurDeRemplissage, taille));
                break;
            case 'above':
                objets.push(latexParCoordonnees(unPoint.nom, x, y + offset, color, largeur, hauteur, couleurDeRemplissage, taille));
                break;
            case 'above right':
                objets.push(latexParCoordonnees(unPoint.nom, x + offset, y + offset, color, largeur, hauteur, couleurDeRemplissage, taille));
                break;
            case 'below left':
                objets.push(latexParCoordonnees(unPoint.nom, x - offset, y - offset, color, largeur, hauteur, couleurDeRemplissage, taille));
                break;
...
export function labelPoint(...args) {
    ...
    for (const unPoint of points) {
        ...
        if (unPoint instanceof PointAbstrait || unPoint instanceof PointAbstrait) {
            ...
            switch (unPoint.positionLabel) {
                case 'left':
                    objets.push(texteParPosition(unPoint.nom, x - 10 / context.pixelsParCm, y, 0, color[0], taille, 'milieu', true, 1));
                    break;
                case 'right':
                    objets.push(texteParPosition(unPoint.nom, x + 10 / context.pixelsParCm, y, 0, color[0], taille, 'milieu', true, 1));
                    break;
                case 'below':
                    objets.push(texteParPosition(unPoint.nom, x, y - 10 / context.pixelsParCm, 0, color[0], taille, 'milieu', true, 1));
                    break;
                case 'above':
                    objets.push(texteParPosition(unPoint.nom, x, y + 10 / context.pixelsParCm, 0, color[0], taille, 'milieu', true, 1));
                    break;
                case 'above left':
                    objets.push(texteParPosition(unPoint.nom, x - 10 / context.pixelsParCm, y + 10 / context.pixelsParCm, 0, color[0], taille, 'milieu', true, 1));
                    break;
                case 'above right':
                    objets.push(texteParPosition(unPoint.nom, x + 10 / context.pixelsParCm, y + 10 / context.pixelsParCm, 0, color[0], taille, 'milieu', true, 1));
                    break;
                ...
                case 'below right':
                    ...
                    break;
                default:
                    objets.push(texteParPosition(unPoint.nom, x, y, 0, color[0], taille, 'milieu', true, 1));
                    break;
            }
        }
    }
...
export class TexteParPoint extends ObjetMathalea2D {
    constructor(texte, A, orientation = 0, color = 'black', scale = 1, ancrageDeRotation = 'milieu', mathOn = false, opacite = 1) {
        super();
        ...
        if (typeof texte !== 'string') {
            if (!Number.isNaN(texte))
                texte = stringNombre(texte, 3);
        }
    ...
    svg(coeff) {
        if (this.texte.charAt(0) === '$') {
            ...
            return latex2d(this.texte.substring(1, this.texte.length - 1), this.point.x, this.point.y, {
                color: this.color[0],
            ...
            code = `<text ${style} x="${this.point.xSVG(coeff)}" y="${this.point.ySVG(coeff)}" text-anchor = "${anchor}" dominant-baseline = "central" fill="${this.couleurDeRemplissage[0]}" transform="rotate(${String(this.orientation)} ${this.point.xSVG(coeff)} ${this.point.ySVG(coeff)})" id="${this.id}" >${this.texte}</text>\n `;
            return code;
        }
    }
    tikz() {
        if (this.texte.charAt(0) === '$') {
            ...
            code = `\\draw [color=${this.color[1]}] (${String(arrondi(this.point.x))},${String(arrondi(this.point.y))}) node[anchor = ${anchor},scale=${String(this.scale)}, rotate = ${String(-this.orientation)}] {${this.texte}};`;
            return code;
        }
        else {
            let code = '';
            ...
            if (this.contour) {
                ...
                if (colorFill === '') {
                    ...
                    code = `\\draw (${arrondi(this.point.x)},${arrondi(this.point.y)}) node[anchor = ${anchor},scale=${String(this.scale)}, rotate = ${String(-this.orientation)}, text opacity=${this.opaciteDeRemplissage}] {\\textcolor{white}{\\contour{${colorContour}}{${this.texte}}}};`;
                }
                else {
                    // Sinon : le texte intérieur a la couleur spécifiée
                    code = `\\draw (${arrondi(this.point.x)},${arrondi(this.point.y)}) node[anchor = ${anchor},scale=${String(this.scale)}, rotate = ${String(-this.orientation)}, text opacity=${this.opaciteDeRemplissage}] {\\textcolor{${colorFill}}{\\contour{${colorContour}}{${this.texte}}}};`;
                }
            }
            else {
                code = `\\draw [${this.color[1] !== '' ? `color=${this.color[1]}` : ''}] (${arrondi(this.point.x)},${arrondi(this.point.y)}) node[anchor = ${anchor},scale=${String(this.scale)}, rotate = ${String(-this.orientation)}] {${this.texte}};`;
            }
            return code;
        }
    }
 ...
 * @author Rémi Angot rectifié par Jean-Claude Lhote
 */
export function texteParPoint(texte, A, orientation = 0, color = 'black', scale = 1, ancrageDeRotation = 'milieu', mathOn = false, opacite = 1) {
...
export class TexteParPointEchelle extends ObjetMathalea2D {
    constructor(texte, A, orientation = 0, color = 'black', scale = 1, ancrageDeRotation = 'milieu', mathOn = false, scaleFigure) {
        super();
        if (typeof orientation !== 'number') {
            if (orientation === 'milieu' || typeof orientation === 'string') {
                ancrageDeRotation = orientation;
    ...
    svg(coeff) {
        if (this.texte.charAt(0) === '$') {
            ...
            if (this.contour)
                ...
                style += ` style="font-size:${this.taille}px;fill:${this.color[0]};fill-opacity:${this.opacite};${this.gras ? 'font-weight:bolder' : ''}" `;
            return `<text ${style} x="${this.point.xSVG(coeff)}" y="${this.point.ySVG(coeff)}" text-anchor = "${svgAncrages[this.ancrageDeRotation]}" dominant-baseline = "central" fill="${this.color[0]}" transform="rotate(${String(this.orientation)} ${this.point.xSVG(coeff)} ${this.point.ySVG(coeff)})" id="${this.id}" >${this.texte}</text>\n `;
        }
    }
    tikz() {
        ...
        return `\\draw ${this.color[0] === 'black' ? '' : `[color=${this.color[1]}]`} (${String(arrondi(this.point.x))},${String(arrondi(this.point.y))}) node[anchor = ${tikzAncrages[this.ancrageDeRotation]}, rotate = ${String(-this.orientation)}, scale=${(this.scale * this.scaleFigure * 1.25).toFixed(2)}] {${copyThistexte}};`;
    }
}
export function texteParPointEchelle(texte, A, orientation = 0, color = 'black', scale = 1, ancrageDeRotation = 'milieu', mathOn = false, scaleFigure = 1) {
    return new TexteParPointEchelle(texte, A, orientation, color, scale, ancrageDeRotation, mathOn, scaleFigure);
}
export function texteParPositionEchelle(texte, x, y, orientation = 0, color = 'black', scale = 1, ancrageDeRotation = 'milieu', mathOn = false, scaleFigure = 1) {
    return texteParPointEchelle(texte, pointAbstrait(arrondi(x, 2), arrondi(y, 2), '', 'center'), orientation, color, scale, ancrageDeRotation, mathOn, scaleFigure);
}
/**
 ...
 * @author Rémi Angot
 */
export function texteParPosition(texte, x, y, orientation = 0, color = 'black', scale = 1, ancrageDeRotation = 'milieu', mathOn = false, opacite) {
    ...
    if (texte[0] === '$') {
        ...
        return new TexteParPoint(texte, pointAbstrait(x, y, ''), orientation, color, scale, ancrageDeRotation, mathOn, opacite);
    }
}
/**
...
export class LatexParCoordonnees extends ObjetMathalea2D {
    constructor(texte, x, y, color, largeur, hauteur, colorBackground = '', tailleCaracteres = 8) {
        var _a, _b;
        super();
        this.x = x;
        this.y = y;
        ...
        this.bordures = [
            this.x - ((_a = this.texte.length) !== null && _a !== void 0 ? _a : 0) * 0.15,
            this.y - 0.02 * this.hauteur,
            this.x + ((_b = this.texte.length) !== null && _b !== void 0 ? _b : 0) * 0.15,
            this.y + 0.02 * this.hauteur,
        ];
        if (tailleCaracteres > 19)
            this.taille = '\\huge';
        else if (tailleCaracteres > 16)
            this.taille = '\\LARGE';
        else if (tailleCaracteres > 13)
            this.taille = '\\Large';
        else if (tailleCaracteres > 11)
            this.taille = '\\large';
        else if (tailleCaracteres < 6)
            this.taille = '\\tiny';
        else if (tailleCaracteres < 8)
            this.taille = '\\scriptsize';
        else if (tailleCaracteres < 9)
            this.taille = '\\footnotesize';
        else if (tailleCaracteres < 10)
            this.taille = '\\small';
        else
            this.taille = '\\normalsize';
        if (typeof this.texte !== 'string')
            throw Error(`Vous n'avez pas passer un string à latexParCoordonnees() : ${this.texte}`);
 ...
 */
export function latexParCoordonnees(texte, x, y, color = 'black', largeur = 50, hauteurLigne = 20, colorBackground = '', tailleCaracteres = 8) {
    if (texte === '')
        texte = '\\phantom{ }';
...
export class LatexParCoordonneesBox extends ObjetMathalea2D {
    constructor(texte, x, y, color, largeur, hauteur, colorBackground, tailleCaracteres = 8, options) {
        super();
        this.taille = 'footnotesize';
        this.x = x;
        this.y = y;
...
export function latexParCoordonneesBox(texte, x, y, color = 'black', largeur = 50, hauteurLigne = 20, colorBackground = 'white', tailleCaracteres = 8, options) {
    if (texte === '')
        texte = '\\phantom{ }'; // pour éviter les latex vides
...
export class Latex2d extends ObjetMathalea2D {
     ...
     *
     */
    constructor(latex, x, y, { color = 'black', backgroundColor = '', letterSize = 'normalsize', orientation = 0, opacity = 1, gras = false, }) {
        super();
        this.col = color;
 ...
 *
 */
export function latex2d(latex, x, y, { color, backgroundColor, letterSize, orientation, opacity, gras, }) {
    ...
    backgroundColor =
        backgroundColor == null ||
            ...
            ? ''
            : backgroundColor;
    letterSize = letterSize !== null && letterSize !== void 0 ? letterSize : 'normalsize';
    orientation = orientation !== null && orientation !== void 0 ? orientation : 0;
    gras = gras !== null && gras !== void 0 ? gras : false;
...
export function tailleDeNbVersLatex(taille) {
    if (taille > 1.9) {
        return 'huge';
    }
    else if (taille > 1.6) {
