...
import { buildImagesUrlsList, doesLatexNeedsPics, getExosContentList, getPicsNames, } from './Latex';
export function downloadZip(filesUrls, zipFileName) {
    const zip = new JSZip();
    let count = 0;
    filesUrls.forEach((url) => {
        JSZipUtils.getBinaryContent(url, (err, data) => {
            ...
            zip.file(fileName, data !== null && data !== void 0 ? data : '', { binary: true });
            count++;
 ...
 * @author sylvain
 */
export async function downloadTexWithImagesZip(zipFileName, latexFile, exercices) {
    const zip = new JSZip();
    ...
    if (withImages) {
        ...
        urls.forEach((url) => {
            JSZipUtils.getBinaryContent(url, (err, data) => {
                ...
                const splitUrl = url.split('/');
                const fileName = splitUrl[splitUrl.length - 1];
                imagesFolder === null || imagesFolder === void 0 ? void 0 : imagesFolder.file(fileName, data !== null && data !== void 0 ? data : '', { binary: true });
                count++;
