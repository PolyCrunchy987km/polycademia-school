...
import { combinaisonListes, combinaisonListesSansChangerOrdre, compteOccurences, enleveDoublonNum, } from '../lib/outils/arrayOutils';
import { arrondi, rangeMinMax } from '../lib/outils/nombres';
import { context } from './context';
// Garde structurel pour détecter une FractionEtendue
const isFractionEtendue = (x) => typeof x === 'object' &&
    x !== null &&
...
export function listeQuestionsToContenu(exercice) {
    ...
    if (!context.isAmc) {
        exercice.contenu =
            texConsigne(exercice.consigne) +
                ...
                texMulticols(texEnumerate(exercice.listeQuestions, exercice.spacing), exercice.nbCols);
    }
    exercice.contenuCorrection =
        texConsigne('') +
            texIntroduction(exercice.consigneCorrection) +
            texMulticols(texEnumerate(exercice.listeCorrections, exercice.spacingCorr), exercice.nbColsCorr);
    ...
    exercice.contenu = (_a = exercice.contenu) === null || _a === void 0 ? void 0 : _a.replace(/\\\\\n*/g, '\\\\\n');
}
/**
...
export function listeQuestionsToContenuSansNumero(exercice, retourCharriot = true) {
    ...
    if (supprimerReferenceCheckbox &&
        supprimerReferenceCheckbox instanceof HTMLInputElement &&
        supprimerReferenceCheckbox.checked === true) {
        exercice.contenu =
            texConsigne(exercice.consigne) +
                texIntroduction(exercice.introduction) +
                texMulticols(texParagraphe(exercice.listeQuestions, exercice.spacing, retourCharriot), exercice.nbCols);
    }
    else {
        exercice.contenu =
            texConsigne(exercice.consigne) +
    ...
    exercice.contenuCorrection =
        texConsigne(exercice.consigneCorrection) +
            texMulticols(texParagraphe(exercice.listeCorrections, exercice.spacingCorr, retourCharriot), exercice.nbColsCorr);
}
/**
...
export function contraindreValeur(min, max, valeur, defaut) {
    ...
    if (isNaN(min) || isNaN(max) || isNaN(defaut)) {
        throw Error(`Erreur dans contraindreValeur : un des paramètres de contrainte est NaN : ${[
            ...
            valeur,
            defaut,
        ].reduce((accu, value, index) => String(accu) +
 ...
 */
export function gestionnaireFormulaireTexte(params) {
    let { saisie, min = 1, max, defaut, listeOfCase, shuffle = true, nbQuestions, melange = 0, enleveDoublons = false, exclus, } = params;
    if (exclus) {
        exclus = exclus.filter((element) => element >= min && element <= max);
    }
    if (max == null || isNaN(max) || max < min)
        throw Error('La fonction gestionnaireFormulaireTexte réclame un paramètre max de type number');
    if (defaut == null ||
        isNaN(defaut) ||
        (defaut < min && defaut !== melange) ||
        (defaut > max && defaut !== melange))
        throw Error(`La fonction gestionnaireFormulaireTexte réclame un paramètre defaut (ici, ${defaut}) compris entre min (ici, ${min}) et max (ici, ${max})`);
    let listeIndex, listeIndexProvisoire;
    listeIndex = [];
    if (typeof saisie === 'boolean' ||
        saisie === '' ||
        saisie === null ||
        saisie === undefined) {
        ...
        if (typeof saisie === 'number' || Number.isInteger(saisie)) {
            ...
            listeIndex = [
                contraindreValeur(Math.min(min, melange !== null && melange !== void 0 ? melange : min), Math.max(max, melange !== null && melange !== void 0 ? melange : max), saisie, defaut),
            ];
        }
        else {
            ...
            for (let i = 0; i < listeIndexProvisoire.length; i++) {
                ...
                if (!isNaN(parseInt(listeIndexProvisoire[i]))) {
                    listeIndex.push(contraindreValeur(Math.min(min, melange !== null && melange !== void 0 ? melange : min), Math.max(max, melange !== null && melange !== void 0 ? melange : max), parseInt(listeIndexProvisoire[i]), defaut));
                } // parseInt en fait un tableau d'entiers
            }
        }
    }
    if (listeIndex.length === 0)
        listeIndex = [defaut]; // EE : Le cas où finalement listeIndex est vide car la saisie n'était pas un paramètre attendu.
    ...
    if (enleveDoublons)
        listeIndex = enleveDoublonNum(listeIndex);
    if (Array.isArray(listeOfCase)) {
        ...
        if (listeOfCase.length < Max)
            throw Error('La liste de cas fournie ne contient pas assez de valeurs par rapport à max');
...
export function entreDeux(a, b) {
    if (a < b)
        return arrondi(a + ((b - a) * randint(10, 90)) / 100, 2);
    else
        return arrondi(b + ((a - b) * randint(10, 90)) / 100, 2);
}
/**
...
export function estentier(a, tolerance = epsilon) {
    if (typeof a !== 'number')
        window.notify('Erreur dans estEntier()', { a });
 ...
 */
export const randFloat = (a, b, precision = 3) => {
    const diff = b - a;
    return (Math.round((a + Math.random() * diff) * 10 ** precision) / 10 ** precision);
};
/**
 ...
 * @author Rémi Angot
 */
export function texParagraphe(liste, spacing = 0, retourCharriot) {
    let result = '';
    if (spacing > 1) {
        result = `\\begin{spacing}{${spacing}}\n`;
    }
...
export function texConsigne(consigne) {
    return ((consigne != null && typeof consigne === 'string'
...
export function itemize(tableauDeTexte) {
    if (Array.isArray(tableauDeTexte) &&
