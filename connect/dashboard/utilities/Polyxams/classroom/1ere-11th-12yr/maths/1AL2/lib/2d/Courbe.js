...
export class Courbe extends ObjetMathalea2D {
    constructor(f, { repere, color = 'black', epaisseur = 2, step = false, xMin, xMax, yMin, yMax, xUnite = 1, yUnite = 1, usePgfplots = false, fLatex, } = {}) {
        var _a, _b, _c, _d, _e;
        super();
        this.samples = 100;
        this.objets = [];
        ...
        this.fLatex = fLatex;
        if (repere == null) {
            throw Error('Erreur dans Courbe : Il faut préciser le repère dans lequel tracer la courbe');
        }
        const xmin = xMin == null ? (xMin = (_a = repere.xMin) !== null && _a !== void 0 ? _a : 0) : xMin;
        const xmax = xMax == null ? (xMax = (_b = repere.xMax) !== null && _b !== void 0 ? _b : 0) : xMax;
        const ymin = yMin == null ? (yMin = (_c = repere.yMin) !== null && _c !== void 0 ? _c : 0) : yMin;
        const ymax = yMax == null ? (yMax = (_d = repere.yMax) !== null && _d !== void 0 ? _d : 0) : yMax;
        this.xmin = xmin;
        this.xmax = xmax;
        this.ymin = ymin;
        this.ymax = ymax;
        this.bordures = ((_e = repere === null || repere === void 0 ? void 0 : repere.bordures) !== null && _e !== void 0 ? _e : [0, 0, 0, 0]);
        ...
        let lastFiniteX = null; // Dernier x ayant fourni une valeur finie
        let lastOutOfBoundsX = null; // Dernier x hors cadre mais fini
        for (let x = xmin; inferieurouegal(x, xMax !== null && xMax !== void 0 ? xMax : 10); x += pas) {
            if (x > xmax)
                x = xmax; // normalement x<xmax... mais inférieurouegal ne compare qu'à 0.0000001 près, on peut donc avoir xmax+epsilon qui sort de l'intervalle de déf
            const y = Number(f(x));
            if (isFinite(y)) {
                ...
                if (lastFiniteX !== null) {
                    ...
                    let xSmall = lastFiniteX + smallStep;
                    while (xSmall < x) {
    ...
    tikz(axisYMin, axisYMax, _axisXMin, _axisXMax) {
        if (this.usePgfplots && this.fLatex) {
            ...
            const yDomainMin = (axisYMin !== undefined ? axisYMin * 20 : this.ymin * this.yunite * 20).toFixed(3);
            const yDomainMax = (axisYMax !== undefined ? axisYMax * 20 : this.ymax * this.yunite * 20).toFixed(3);
 ...
 */
// JSDOC Validee par EE Aout 2022
export function courbe(f, { repere, color = 'black', epaisseur = 2, step = false, xMin, xMax, yMin, yMax, xUnite = 1, yUnite = 1, usePgfplots = false, fLatex, } = {}) {
    return new Courbe(f, {
        repere,
        color,
        epaisseur,
