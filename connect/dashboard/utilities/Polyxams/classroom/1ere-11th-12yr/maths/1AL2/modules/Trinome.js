...
export default class Trinome {
    ...
    constructor(a, b, c) {
        ...
        this.precision = 3;
        if (typeof a === 'number')
            this.a = new FractionEtendue(a, 1);
        else
            this.a = a;
        if (typeof b === 'number')
            this.b = new FractionEtendue(b, 1);
        else
            this.b = b;
        if (typeof c === 'number')
            this.c = new FractionEtendue(c, 1);
        else
            this.c = c;
    ...
    defFormeFactorisee(a, x1, x2) {
        if (a instanceof FractionEtendue === false)
            a = new FractionEtendue(a, 1);
        if (x1 instanceof FractionEtendue === false)
            x1 = new FractionEtendue(x1, 1);
        if (x2 instanceof FractionEtendue === false)
            x2 = new FractionEtendue(x2, 1);
        this.a = a;
    ...
    defFormeFactorisee2(k, a, b, c, d) {
        if (k instanceof FractionEtendue === false)
            k = new FractionEtendue(k, 1);
        if (a instanceof FractionEtendue === false)
            a = new FractionEtendue(a, 1);
        if (b instanceof FractionEtendue === false)
            b = new FractionEtendue(b, 1);
        if (c instanceof FractionEtendue === false)
            c = new FractionEtendue(c, 1);
        if (d instanceof FractionEtendue === false)
            d = new FractionEtendue(d, 1);
    ...
    isEqual(trinome) {
        return (this.a.isEqual(trinome.a) &&
    ...
    image(x) {
        if (x instanceof FractionEtendue === false)
            x = new FractionEtendue(x, 1);
        return this.a
            .produitFraction(x)
            .produitFraction(x)
    ...
    texCalculImage(x) {
        ...
        if (this.b.valeurDecimale !== 0) {
            ...
            else if (this.b.valeurDecimale === -1)
                result += `- ${x.texFSP} `;
            else
                result += `${this.b.simplifie().texFractionSignee} \\times ${x.texFSP} `;
        }
        if (this.c.valeurDecimale !== 0)
            result += `${this.c.texFractionSignee}`;
            ...
            if (this.x1 instanceof FractionEtendue)
                ...
                if (this.discriminant.estEntiere) {
                    ...
                    if (r0 !== 1 &&
                        ...
                        if (this.a.valeurDecimale > 0) {
                            const num = this.b.entierDivise(r0).simplifie().oppose().texFraction +
                                `- \\sqrt{${r1}}`;
                            const den = (2 * this.a.valeurDecimale) / r0;
                            result += den === 1 ? `=${num}` : `=\\dfrac{${num}}{${den}}`;
                        }
                        else {
                            const num = this.b.entierDivise(r0).simplifie().texFraction +
                                `- \\sqrt{${r1}}`;
                            const den = (-2 * this.a.valeurDecimale) / r0;
                            result += den === 1 ? `=${num}` : `=\\dfrac{${num}}{${den}}`;
                        }
    ...
    get arrayTexDevelopperFormeCanonique() {
        ...
        if (this.a.valeurDecimale === -1)
            ...
            result1 += this.a.simplifie().texFSD;
    ...
    get arrayTexDevelopperFormeFactorisee() {
        ...
        if (typeof untypedX1 === 'boolean' ||
            typeof untypedX2 === 'boolean' ||
            typeof untypedA === 'boolean')
            throw Error;
        const x1 = typeof untypedX1 === 'number'
            ? new FractionEtendue(untypedX1, 1)
            : untypedX1;
        const x2 = typeof untypedX2 === 'number'
    ...
    get x1() {
        ...
        if (this.a.valeurDecimale > 0) {
            result =
                Math.round(((-this.b.valeurDecimale -
        ...
        if (this.x1 instanceof FractionEtendue)
            ...
            if (this.discriminant.estEntiere &&
                ...
                if (r0 !== 1 &&
                    ...
                    // On peut simplifier par r0
                    if (this.a.valeurDecimale > 0) {
                        const num = (this.b.valeurDecimale !== 0
            ...
            if (this.a.valeurDecimale > 0) {
                const num = (this.b.valeurDecimale !== 0 ? this.b.oppose().texFraction : '') +
                ...
                const num = (this.b.valeurDecimale !== 0 ? this.b.texFraction : '') +
                    (this.discriminant.valeurDecimale > 0
                        ? `-\\sqrt{${this.discriminant.texFraction}}`
                        : '');
                const den = -2 * this.a.valeurDecimale;
    ...
    get texFormeCanonique() {
        ...
        if (this.a.valeurDecimale === -1)
            ...
            if (this.a.den === 1)
                ...
                result = this.a.texFractionSimplifiee;
        }
        if (this.alpha.valeurDecimale === 0)
            result += 'x^2';
        else
