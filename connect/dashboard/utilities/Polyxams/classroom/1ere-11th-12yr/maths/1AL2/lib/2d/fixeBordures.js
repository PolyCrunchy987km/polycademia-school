 ...
 */
export function fixeBordures(objets, { rxmin = -0.5, rymin = -0.5, rxmax = 0.5, rymax = 0.5, rzoom = 1 } = {}) {
    /**
     *
     * @param{number} xmin
     * @param{number} ymin
    ...
    const majBordures = (xmin, ymin, xmax, ymax, objets, borduresTrouvees) => {
        ...
        if (!Array.isArray(objets)) {
            const bordures = (_a = objets.bordures) !== null && _a !== void 0 ? _a : null;
            if (bordures === null || isNaN(bordures[0])) {
                window.notify(`Ìl y a un problème avec les bordures de ${objets.constructor.name}... elles ne sont pas définies !`, { objets });
            }
            else if (!Array.isArray(bordures)) {
                window.notify(`Les bordures de ${objets.constructor.name} ne sont pas un array : ${JSON.stringify(bordures)}`, Object.assign({}, objets));
            }
            else if (bordures.filter((el) => isNaN(el)).length > 0) {
                window.notify(`Les bordures de ${objets.constructor.name} sont bien un array mais contiennent autre chose que des nombres : ${bordures}`, Object.assign({}, objets));
            }
            else {
    ...
    if (!borduresTrouvees) {
        window.notify('fixeBordures : aucun objet ne définit de bordures valides', Object.assign({}, objets));
    }
    return {
