    ...
    for (let i = 0, cpt = 0; i < this.nbQuestions && cpt < 50; ) {
      ...
      switch (listeTypesEquations[i]) {
        case "0solution":
          ...
          texte = `$${rienSi1(a)}x^2${ecritureAlgebriqueSauf1(
            b
          )}x${ecritureAlgebrique(c)}=0$`
          if (b === 0) {
            texte = `$${rienSi1(a)}x^2${ecritureAlgebrique(c)}=0$`
          }
          texteCorr = `$\\Delta = ${ecritureParentheseSiNegatif(
            b
          )}^2-4\\times${ecritureParentheseSiNegatif(
            a
          )}\\times${ecritureParentheseSiNegatif(c)}=${b * b - 4 * a * c}$`
          texteCorr += `<br>$\\Delta<0$ donc l'équation ${texteEnCouleurEtGras(
        ...
        case "1solution": // k(x-x1)^2
          ...
          texteCorr += `<br>$\\Delta=0$ donc l'équation admet ${texteEnCouleurEtGras(
            "une unique solution"
          )}.`
          break
      ...
      if (context.isHtml) {
        ...
        let correctionComplementaire = `Notons $f : x \\mapsto ${rienSi1(
          a
        )}x^2${ecritureAlgebriqueSauf1(b)}x${ecritureAlgebrique(c)}$.`
