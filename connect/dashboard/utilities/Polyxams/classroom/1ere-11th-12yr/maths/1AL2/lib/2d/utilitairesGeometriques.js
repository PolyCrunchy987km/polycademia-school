 ...
 * @return {boolean}
 */
export function estDansTriangle(M, A, B, C) {
 ...
 * @return {boolean}
 */
export function estDansQuadrilatere(M, A, B, C, D) {
 ...
 * @return {boolean}
 */
export function estDansPolygoneConvexe(M, p) {
    const l = p.listePoints.length;
    if (l === 3) {
        return estDansTriangle(M, p.listePoints[0], p.listePoints[1], p.listePoints[2]);
    }
    else {
        ...
        if (estDansTriangle(M, A, B, C))
            return true;
        else
            return estDansPolygoneConvexe(M, p2);
    }
}
/**
 ...
 * @return {boolean}
 */
export function estDansPolygone(M, p) {
    for (const triangle of p.triangulation) {
        if (estDansTriangle(M, triangle[0], triangle[1], triangle[2]))
            return true;
    }
    return false;
}
 ...
 */
export function pointEstSur(M, objet) {
    // Droite (ax + by + c = 0)
    if ('pente' in objet) {
    ...
    if ('extremite1' in objet && 'extremite2' in objet && 'longueur' in objet) {
        const prodvect = (objet.extremite2.x - objet.extremite1.x) * (M.y - objet.extremite1.y) -
            (M.x - objet.extremite1.x) * (objet.extremite2.y - objet.extremite1.y);
        const prodscal = (M.x - objet.extremite1.x) * (objet.extremite2.x - objet.extremite1.x) +
            (M.y - objet.extremite1.y) * (objet.extremite2.y - objet.extremite1.y);
        const prodscalABAB = (objet.extremite2.x - objet.extremite1.x) ** 2 +
            (objet.extremite2.y - objet.extremite1.y) ** 2;
        return (Math.abs(prodvect) <= 0.01 && prodscal >= 0 && prodscal <= prodscalABAB);
    }
...
export const longueur = (A, B, precision = 2) => {
    const dx = B.x - A.x;
    const dy = B.y - A.y;
...
export function angleradian(A, O, B, precision = 2) {
    ...
    return Number(Math.acos((AB ** 2 - OA ** 2 - OB ** 2) / (-2 * OA * OB)).toFixed(precision));
} /**
...
export function estSecant(s, objet) {
    ...
    // Cercle
    if ('rayon' in objet && 'centre' in objet) {
        const R = Number(objet.rayon);
        const C = objet.centre;
    ...
    if ('extremite1' in objet && 'extremite2' in objet) {
        const C = objet.extremite1;
        const D = objet.extremite2;
        ...
        const isOtherSegment = 'longueur' in objet;
        const IOnS = onSegment(I, A, B);
        if (!IOnS)
            return false;
        if (isOtherSegment)
            return onSegment(I, C, D);
 ...
 */
export function normaliseOrientation(orientation) {
