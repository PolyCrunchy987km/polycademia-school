...
const vue = [''];
const recorder = [''];
function paraConvert(param) {
    const par = { uuid: param.uuid };
    if (param.nbQuestions)
        par.nb = param.nbQuestions;
    if (param.type)
        par.t = param.type;
    if (param.exo)
        par.e = param.exo;
    if (param.exoType)
        par.et = param.exoType;
    if (param.cd)
        par.c = param.cd;
    if (param.sup)
        par.s1 = param.sup.replace('true', 't').replace('false', 'f');
    if (param.sup2)
        par.s2 = param.sup2.replace('true', 't').replace('false', 'f');
    if (param.sup3)
        par.s3 = param.sup3.replace('true', 't').replace('false', 'f');
    if (param.sup4)
        par.s4 = param.sup4.replace('true', 't').replace('false', 'f');
    if (param.sup5)
        par.s5 = param.sup5.replace('true', 't').replace('false', 'f');
    if (param.alea)
        par.a = param.alea;
    if (param.interactif)
        par.i = param.interactif;
    if (param.bestScore)
        par.s = param.bestScore;
    return par;
}
globalOptions.subscribe((options) => {
    ...
    if (options) {
        if (options.v === undefined)
            return; // chargement du site
        if (options.recorder === 'capytale' && get(capytaleMode) === 'none')
            return; // capytale pas encore chargé
        ...
        log('vue', vue[0]);
        if (window._paq)
            window._paq.push(['trackEvent', 'Vue', vue[0]]);
        // if (window._paq) window._paq.push(['setCustomDimension', 1, vue[0]])
        if (options.recorder)
            recorder[0] = options.recorder;
    ...
    if (params) {
        ...
        params.forEach((param) => {
            ...
            if (uuids.includes(param.uuid)) {
                ...
                if (window._paq)
                    window._paq.push(['trackEvent', 'Uuid', param.uuid]);
                log('Uuids-' + param.uuid);
            }
            // on met à jour la liste des paramètres avec la vue et le recorder:
            if (vue[0] !== '')
                vueUuids1.push(vue[0] + '-' + param.uuid + (recorder[0] ? '-' + recorder[0] : ''));
...
export function statsCanTracker(recorder, vue) {
