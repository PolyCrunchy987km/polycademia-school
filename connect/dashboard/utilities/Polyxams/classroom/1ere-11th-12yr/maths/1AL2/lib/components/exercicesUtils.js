...
import { isStaticType, } from '../../lib/types/referentiels';
import { mathaleaFormatExercice, mathaleaHandleExerciceSimple, mathaleaHandleParamOfOneExercice, mathaleaLoadExerciceFromUuid, } from '../mathalea';
...
const allStaticReferentiels = Object.assign(Object.assign({}, referentielStaticFR), referentielStaticCH);
 ...
 */
export const buildExercisesList = (filter = []) => {
    const promiseExos = [];
    ...
    for (const paramsExercice of exosParams) {
        ...
        if (isStatic(paramsExercice.uuid)) {
          ...
          <img src="${foundResource.pngCor || ''}" style="width: calc(100% * {zoomFactor}" alt="correction" />`;
                ...
                if (options.setInteractive === '1' && (exo === null || exo === void 0 ? void 0 : exo.interactifReady)) {
                    exo.interactif = true;
                }
                resolve(exo);
            ...
            const p = new Promise((resolve) => {
                ...
                mathaleaLoadExerciceFromUuid(paramsExercice.uuid).then((exo) => {
                    if (typeof exo === 'undefined') {
                        throw new Error("L'exercice correspondant à l'uuid " +
                            paramsExercice.uuid +
                            " n'est pas défini...");
                    }
                    mathaleaHandleParamOfOneExercice(exo, paramsExercice);
...
export const splitExercisesIntoQuestions = (exercices) => {
    let questions = [];
    let consignes = [];
    let corrections = [];
    let consignesCorrections = [];
    const isCorrectionVisible = [];
    const indiceExercice = [];
    const indiceQuestionInExercice = [];
    for (const [k, exercice] of exercices.entries()) {
        ...
        for (let i = 0; i < exercice.listeQuestions.length; i++) {
            consignes.push(`${exercice === null || exercice === void 0 ? void 0 : exercice.consigne} ${(exercice === null || exercice === void 0 ? void 0 : exercice.consigne) && (exercice === null || exercice === void 0 ? void 0 : exercice.introduction) ? '<br>\n' : ''} ${exercice === null || exercice === void 0 ? void 0 : exercice.introduction}`);
        ...
        let newQuestions = exercice.listeQuestions.map(mathaleaFormatExercice);
        let newCorrections = exercice.listeCorrections.map(mathaleaFormatExercice);
