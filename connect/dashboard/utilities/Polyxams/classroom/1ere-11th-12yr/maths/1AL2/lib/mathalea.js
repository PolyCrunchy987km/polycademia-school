...
import { isAnswerValueType, isValeur, } from '../lib/types';
...
import { exercicesParams, freezeUrl, presModeId, updateGlobalOptionsInURL, } from './stores/generalStore';
...
*/
export async function getSvelteComponent(paramsExercice) {
    const urlExercice = uuidToUrl[paramsExercice.uuid];
 ...
 */
export async function mathaleaLoadExerciceFromUuid(uuid) {
        ...
        try {
            ...
            if (isCan === 'can') {
                ...
                if (filename != null && filename.includes('.ts')) {
                    ...
                    if (!loader)
                        throw new Error(`Module "${path}" introuvable`);
                    module = (await loader());
                }
            ...
            if (pathToCheck !== '') {
                ...
                if (url !== '') {
                    ...
                    window.notify(`Load failed: ${url} (exists on server: ${exists.reachable} status: ${exists.status})`, { error, exists });
                }
            }
 ...
 */
export async function mathaleaGetExercicesFromParams(params) {
    const exercices = [];
    for (const param of params) {
        if (param.uuid.substring(0, 4) === 'crpe' ||
            ...
            const infosExerciceStatique = param.uuid.substring(0, 7) === 'evacom_'
            ...
            if (!infosExerciceStatique) {
                throw new Error(`Informations introuvables pour l'exercice statique avec l'UUID : ${param.uuid}`);
            }
            let content = '';
            let contentCorr = '';
            ...
            const annee = 'annee' in infosExerciceStatique
                ? infosExerciceStatique.annee
                : undefined;
            const lieu = 'lieu' in infosExerciceStatique ? infosExerciceStatique.lieu : undefined;
            const mois = 'mois' in infosExerciceStatique ? infosExerciceStatique.mois : undefined;
            const numeroInitial = 'numeroInitial' in infosExerciceStatique
                ? infosExerciceStatique.numeroInitial
                : undefined;
            let examen = '';
            if (param.uuid.substring(0, 4) === 'crpe')
                examen = 'CRPE';
            if (param.uuid.substring(0, 4) === 'dnb_')
                examen = 'DNB';
            if (param.uuid.startsWith('dnbpro_'))
                examen = 'DNBPRO';
            if (param.uuid.substring(0, 4) === 'e3c_')
                examen = 'E3C';
            if (param.uuid.substring(0, 4) === 'bac_')
                examen = 'BAC';
            if (param.uuid.startsWith('sti2d_'))
                examen = 'STI2D';
            if (param.uuid.startsWith('stl_'))
                examen = 'STL';
            if (param.uuid.substring(0, 7) === 'evacom_')
                examen = 'EVACOM';
            exercices.push({
 ...
 */
export function mathaleaHandleParamOfOneExercice(exercice, param) {
    var _a;
    exercice.uuid = param.uuid;
    if (param.nbQuestions)
        exercice.nbQuestions = param.nbQuestions;
    exercice.duration = (_a = param.duration) !== null && _a !== void 0 ? _a : 10;
    if (param.id)
        exercice.id = param.id;
    if (param.sup)
        exercice.sup = mathaleaHandleStringFromUrl(param.sup);
    if (param.sup2)
        exercice.sup2 = mathaleaHandleStringFromUrl(param.sup2);
    if (param.sup3)
        exercice.sup3 = mathaleaHandleStringFromUrl(param.sup3);
    if (param.sup4)
        exercice.sup4 = mathaleaHandleStringFromUrl(param.sup4);
    if (param.sup5)
        exercice.sup5 = mathaleaHandleStringFromUrl(param.sup5);
    if (param.versionQcm !== undefined && exercice instanceof ExerciceSimple)
        exercice.versionQcm = param.versionQcm === '1';
    if (param.interactif)
        exercice.interactif = param.interactif === '1';
    if (param.alea)
        exercice.seed = param.alea;
    if (param.cols !== undefined && param.cols > 1)
        exercice.nbCols = param.cols;
    if (param.cd !== undefined)
        exercice.correctionDetaillee = param.cd === '1';
 ...
 */
export function mathaleaHandleSup(param) {
 ...
 */
export function mathaleaHandleStringFromUrl(text) {
...
export function mathaleaRenderDiv(div, zoom) {
    if (!div)
        return;
    const params = get(globalOptions);
    zoom = zoom !== null && zoom !== void 0 ? zoom : Number(params.z);
    renderKatex(div);
    renderScratchDiv(div !== null && div !== void 0 ? div : document.body);
    if (zoom !== -1) {
        resizeContent(div, zoom);
    }
}
export function renderDiv(HtmlElement, _content) {
 ...
 */
export function mathaleaUpdateUrlFromExercicesParams(params) {
    if (get(globalOptions).recorder === 'capytale') {
        sendToCapytaleMathaleaHasChanged();
    }
    if (get(freezeUrl) === true)
        return;
    if (params === undefined) {
 ...
 * @returns vue
 */
export function mathaleaUpdateExercicesParamsFromUrl(urlString = window.location.href) {
    ...
    try {
        for (const entry of entries) {
            if (entry[0] === 'uuid') {
                indiceExercice++;
                const uuid = entry[1];
                const id = Object.keys(currentRefToUuid).find((key) => {
                ...
                indiceExercice++;
                const id = entry[1];
                const uuid = currentRefToUuid[id];
                if (!newExercisesParams[indiceExercice])
            ...
            if (entry[0] === 'uuid')
                previousEntryWasUuid = true;
            else
                previousEntryWasUuid = false;
        }
    }
    catch (error) {
    ...
    const newExercisesParamsFiltered = newExercisesParams.filter((e) => e.uuid || e.id);
    if (JSON.stringify(get(exercicesParams)) !==
    ...
    if (v === 'can' || get(globalOptions).recorder === 'capytale') {
        canOptions.update((e) => {
            ...
            if (canSolMode === 'gathered')
                e.solutionsMode = 'gathered';
            else
                e.solutionsMode = 'split';
            e.subTitle = canTitle;
            return e;
        });
    }
    if (ds) {
        ...
        if (isIntegerInRange1to4(nbVuesCandidate))
            nbVues = nbVuesCandidate;
        if (isIntegerInRange0to2(flowCandidate))
            flow = flowCandidate;
 ...
 */
export function mathaleaHandleExerciceSimple(exercice, isInteractif, numeroExercice, seed) {
    var _a, _b, _c, _d, _e, _f, _g, _h, _j, _k, _l, _m, _o, _p, _q, _r, _s, _t, _u, _v, _w, _x, _y;
    if (numeroExercice !== undefined)
        exercice.numeroExercice = numeroExercice;
    exercice.reinit();
    exercice.interactif = isInteractif;
    for (let i = 0, cptSecours = 0; i < exercice.nbQuestions && cptSecours < 50;) {
        seedrandom(seed !== null && seed !== void 0 ? seed : String(exercice.seed) + i + cptSecours, { global: true });
        if (exercice.nouvelleVersion &&
                ...
                if (compare != null) {
                    ...
                    if (typeof exercice.reponse !== 'string' &&
                        ...
                        if (exercice.reponse instanceof FractionEtendue) {
                            ...
                            window.notify(`MathaleaHandleExerciceSimple n'a pas réussi à déterminer le type de exercice.reponse, dans ${((_a = exercice === null || exercice === void 0 ? void 0 : exercice.numeroExercice) !== null && _a !== void 0 ? _a : 0) + 1} - ${exercice.titre} ${JSON.stringify(exercice.reponse)}, on Stingifie, mais c'est sans doute une erreur à rectifier`, { exercice: JSON.stringify(exercice) });
                            reponse = {
                        ...
                        reponse = {
                            reponse: {
                                value: typeof exercice.reponse === 'number'
                    ...
                    handleAnswers(exercice, i, reponse, {
                        formatInteractif: (_b = exercice.formatInteractif) !== null && _b !== void 0 ? _b : 'mathlive',
                    }); /// // PROCHAIN LA : La partie ci-dessus sera à supprimer quand il n'y aura plus de this.compare
                }
                else if (isValeur(exercice.reponse) &&
                    ...
                    setReponse(exercice, i, String(exercice.reponse), {
                        formatInteractif: (_c = exercice.formatInteractif) !== null && _c !== void 0 ? _c : 'calcul',
                    });
                }
            }
            else {
                if (exercice.formatInteractif !== 'qcm')
                    window.notify("Un exercice simple doit avoir un this.reponse sauf si c'est un qcm", { exercice: JSON.stringify(exercice) });
            }
            if (exercice.formatInteractif !== 'fillInTheBlank') {
                if (exercice.formatInteractif === 'qcm' ||
                    (exercice instanceof ExerciceSimple &&
                        ...
                        exercice.versionQcm)) {
                    if (exercice instanceof ExerciceSimple &&
                        exercice.distracteurs.length > 0) {
                        ...
                        if (typeof exercice.reponse === 'string' ||
                            ...
                            exercice.reponse instanceof Hms ||
                            (Array.isArray(exercice.reponse) &&
                                exercice.reponse.every((r) => typeof r === 'string' ||
                                    typeof r === 'number' ||
                            ...
                            const qcmData = buildSimpleVersionQcm(exercice, i, {
                                question: (_d = exercice.question) !== null && _d !== void 0 ? _d : '',
                                correction: (_e = exercice.correction) !== null && _e !== void 0 ? _e : '',
                                reponse: exercice.reponse,
                                distracteurs: exercice.distracteurs,
                                options: (_f = exercice.versionQcmOptions) !== null && _f !== void 0 ? _f : { radio: true },
                            });
                    ...
                    exercice.question = (_g = exercice.question) === null || _g === void 0 ? void 0 : _g.replace(`id="ex${n}Q0"`, `id="ex${n}Q${i}"`);
                    exercice.question = (_h = exercice.question) === null || _h === void 0 ? void 0 : _h.replace(`checkEx${n}Q0"`, `checkEx${n}Q${i}"`);
                    exercice.listeQuestions.push((_j = exercice.question) !== null && _j !== void 0 ? _j : '');
                }
                else if (exercice.formatInteractif === 'MetaInteractif2d') {
                    const n = exercice.numeroExercice;
                    if (exercice.question != null) {
                        const inputsIds = exercice.question.matchAll(/id="MetaInteractif2dEx\d+Q\d+field(\d+)"/g);
                        for (const match of inputsIds) {
                            exercice.question = exercice.question.replace(`id="MetaInteractif2dEx${n}Q0field${match[1]}"`, `id="MetaInteractif2dEx${n}Q${i}field${match[1]}"`);
                        }
                        exercice.question = exercice.question.replace(`id="resultatCheckEx${n}Q0"`, `id="resultatCheckEx${n}Q${i}"`);
                        exercice.question = exercice.question.replace(`id="feedbackEx${n}Q0"`, `id="feedbackEx${n}Q${i}"`);
                        exercice.listeQuestions.push((_k = exercice.question) !== null && _k !== void 0 ? _k : '');
                    }
                }
                else if (exercice.formatInteractif === 'svgSelection') {
                    const n = exercice.numeroExercice;
                    exercice.question = (_l = exercice.question) === null || _l === void 0 ? void 0 : _l.replace(`id="svgSelectionEx${n}Q0"`, `id="svgSelectionEx${n}Q${i}"`);
                    exercice.question = (_m = exercice.question) === null || _m === void 0 ? void 0 : _m.replace(`checkSvgSelectionEx${n}Q0"`, `checkSvgSelectionEx${n}Q${i}"`);
                    exercice.listeQuestions.push((_o = exercice.question) !== null && _o !== void 0 ? _o : '');
                }
                else {
                    exercice.listeQuestions.push(exercice.question +
                        ajouteChampTexteMathLive(exercice, i, String(exercice.formatChampTexte), exercice.optionsChampTexte || {}));
                }
            }
            else {
                ...
                if (typeof exercice.reponse === 'object' &&
                    ...
                    if (typeof exercice.reponse === 'string' ||
                        typeof exercice.reponse === 'number') {
                        handleAnswers(exercice, i, { champ1: { value: (_p = exercice.reponse) !== null && _p !== void 0 ? _p : '' } }, { formatInteractif: 'fillInTheBlank' });
                    }
                }
            }
            exercice.listeCorrections.push((_q = exercice.correction) !== null && _q !== void 0 ? _q : '');
            (_r = exercice.listeCanEnonces) === null || _r === void 0 ? void 0 : _r.push((_s = exercice.canEnonce) !== null && _s !== void 0 ? _s : '');
            (_t = exercice.listeCanReponsesACompleter) === null || _t === void 0 ? void 0 : _t.push((_u = exercice.canReponseACompleter) !== null && _u !== void 0 ? _u : '');
            (_v = exercice.listeCanLiees) === null || _v === void 0 ? void 0 : _v.push((_w = exercice.canLiee) !== null && _w !== void 0 ? _w : '');
            (_x = exercice.listeCanNumerosLies) === null || _x === void 0 ? void 0 : _x.push((_y = exercice.canNumeroLie) !== null && _y !== void 0 ? _y : '');
            cptSecours = 0;
            i++;
        }
        else {
...
export function getDistracteurs(exerciceSimple) {
    ...
    const distracteursNonSolutions = distracteursUniques.filter((distracteur) => {
        const reponse = exerciceSimple.reponse;
        if (reponse == null) {
        ...
        if (isAnswerValueType(reponse)) {
            ...
            if (reponseReponse !== undefined)
                value = reponseReponse.value;
        }
        if (value === undefined) {
 ...
 */
export function mathaleaGenerateSeed({ includeUpperCase = true, includeNumbers = true, length = 4, startsWithLowerCase = false, } = {}) {
    let a = 10;
...
export async function getExercisesFromExercicesParams() {
    ...
    for (const paramsExercice of get(exercicesParams)) {
        ...
        exercise.duration = (_a = paramsExercice.duration) !== null && _a !== void 0 ? _a : 10;
