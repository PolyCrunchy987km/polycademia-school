...
export class PapierPointe extends ObjetMathalea2D {
    constructor({ xmin = -10, xmax = 10, ymin = -10, ymax = 10, xstep = 1, ystep = 1, type = 'quad', pointColor = 'black', pointRayon = 0.05, opacite = 1, opaciteDeRemplissage = 1, }) {
        super();
        this.tikz = () => {
            let code = '';
                ...
                for (let x = xmin; x <= xmax; x += xstep2) {
                    for (let y = ymin; y <= ymax; y += 1.5 * ystep1) {
                        ...
                        if (stepper) {
                            ...
                            this.plots.push(plot(x + xstep1, y + ystep1 / 2, {
                                rayon: pointRayon,
                                couleur: pointColor,
                                opacite,
                                couleurDeRemplissage: context.isHtml ? 'none' : '',
                            ...
                            this.plots.push(plot(x + xstep1, y + ystep1 * 1.5, {
                                rayon: pointRayon,
                                couleur: pointColor,
                                opacite,
                                couleurDeRemplissage: context.isHtml ? 'none' : '',
                            ...
                            this.plots.push(plot(x, y + ystep1 / 2, {
                                rayon: pointRayon,
                                couleur: pointColor,
                                opacite,
                                couleurDeRemplissage: context.isHtml ? 'none' : '',
                ...
                for (let x = xmin; x <= xmax; x = x + xstep2) {
                    for (let y = ymin; y <= ymax; y = y + 1.5 * ystep1) {
                        ...
                        if (stepper) {
                            ...
                            this.plots.push(plot(x, y + ystep1, {
                                rayon: pointRayon,
                                couleur: pointColor,
                                opacite,
                                couleurDeRemplissage: context.isHtml ? 'none' : '',
                            ...
                            this.plots.push(plot(x + xstep1, y + ystep1, {
                                rayon: pointRayon,
                                couleur: pointColor,
                                opacite,
                                couleurDeRemplissage: context.isHtml ? 'none' : '',
...
export function papierPointe({ xmin = -10, xmax = 10, ymin = -10, ymax = 10, xstep = 1, ystep = 1, type = 'quad', pointColor = 'black', pointRayon = 0.05, opacite = 0.4, opaciteDeRemplissage = 0.4, }) {
...
export class Repere extends ObjetMathalea2D {
    constructor({ xUnite = 1, yUnite = 1, xMin = -10, xMax = 10, yMin = -10, yMax = 10, axeXisVisible = true, axeYisVisible = true, axesEpaisseur = 1.2, axesCouleur = 'black', axeXStyle = '->', axeYStyle = '->', thickEpaisseur = 1.2, thickHauteur = 0.13, thickCouleur = axesCouleur, xThickDistance = 1, xThickListe = [], xThickMin = xMin + xThickDistance, xThickMax = xMax - xThickDistance, yThickDistance = 1, yThickListe = [], yThickMin = yMin + yThickDistance, yThickMax = yMax - yThickDistance, labelsXareVisible = true, labelsYareVisible = true, xLabelDistance = xThickDistance, xLabelListe = [], xLabelMin = xThickMin, xLabelMax = xThickMax, yLabelDistance = yThickDistance, yLabelListe = [], yLabelMin = yThickMin, yLabelMax = yThickMax, precisionLabelX = 1, precisionLabelY = 1, xLabelEcart = 0.5, yLabelEcart = 0.5, xLegende = '', xLegendePosition = [], yLegende = '', yLegendePosition = [], grille = true, grilleDistance = 1, grilleCouleur = 'black', grilleOpacite = 0.4, grilleEpaisseur = 1, grilleSecondaire = false, grilleSecondaireDistance = 1, grilleSecondaireCouleur = 'gray', grilleSecondaireOpacite = 0.3, grilleSecondaireEpaisseur = 1, grilleX = grille, grilleXListe = [], grilleXDistance = grilleDistance, grilleXMin = xMin, grilleXMax = xMax, grilleXCouleur = grilleCouleur, grilleXOpacite = grilleOpacite, grilleY = grille, grilleYListe = [], grilleYDistance = grilleDistance, grilleYMin = yMin, grilleYMax = yMax, grilleYCouleur = grilleCouleur, grilleYOpacite = grilleOpacite, grilleSecondaireX = grilleSecondaire, grilleSecondaireXListe = [], grilleSecondaireXDistance = grilleSecondaireDistance, grilleSecondaireXMin = xMin, grilleSecondaireXMax = xMax, grilleSecondaireXCouleur = grilleSecondaireCouleur, grilleSecondaireXOpacite = grilleSecondaireOpacite, grilleSecondaireY = grilleSecondaire, grilleSecondaireYListe = [], grilleSecondaireYDistance = grilleSecondaireDistance, grilleSecondaireYMin = yMin, grilleSecondaireYMax = yMax, grilleSecondaireYCouleur = grilleSecondaireCouleur, grilleSecondaireYOpacite = grilleSecondaireOpacite, }) {
        super();
        this.isRepere = true;
        this.labelsXareVisible = true;
        this.labelsYareVisible = true;
        ...
        this.yMin = yMin;
        this.yMax = yMax;
        this.grilleXMin = grilleXMin !== null && grilleXMin !== void 0 ? grilleXMin : xMin;
        this.grilleXMax = grilleXMax !== null && grilleXMax !== void 0 ? grilleXMax : xMax;
        this.grilleYMin = grilleYMin !== null && grilleYMin !== void 0 ? grilleYMin : yMin;
        this.grilleYMax = grilleYMax !== null && grilleYMax !== void 0 ? grilleYMax : yMax;
        if (thickHauteur === 0) {
            xThickListe = false;
            yThickListe = false;
        }
        ...
        if (axeXisVisible)
            this.objets.push(axeX);
        if (axeYisVisible)
            this.objets.push(axeY);
            ...
            if (grilleSecondaireYListe.length === 0) {
                ...
                if (grilleSecondaireYMin < 0 && grilleSecondaireYMax > 0) {
                    ...
                    for (let y = grilleSecondaireYDistance / yUnite; y < Math.max(-grilleSecondaireYMin, grilleSecondaireYMax); y += grilleSecondaireYDistance / yUnite) {
                        if (y <= grilleSecondaireYMax)
                            grilleSecondaireYListe.push(y);
                        if (y <= -grilleSecondaireYMin)
                            grilleSecondaireYListe.push(-y);
                    }
                }
            ...
            if (grilleSecondaireXListe.length === 0) {
                ...
                if (grilleSecondaireXMin < 0 && grilleSecondaireXMax > 0) {
                    ...
                    for (let x = grilleSecondaireXDistance / xUnite; x < Math.max(-grilleSecondaireXMin, grilleSecondaireXMax); x += grilleSecondaireXDistance / xUnite) {
                        if (x <= grilleSecondaireXMax)
                            grilleSecondaireXListe.push(x);
                        if (x <= -grilleSecondaireXMin)
                            grilleSecondaireXListe.push(-x);
                    }
                }
        ...
        // LES THICKS
        if (axeXisVisible) {
            if ((typeof xThickListe === 'boolean' && xThickListe) ||
            ...
            else if (typeof xThickListe === 'boolean')
                xThickListe = [];
        ...
        if (axeYisVisible) {
            if ((typeof yThickListe === 'boolean' && yThickListe) ||
            ...
            else if (typeof yThickListe === 'boolean')
                yThickListe = [];
        ...
        if (labelsXareVisible) {
            if ((typeof xLabelListe === 'boolean' && xLabelListe) ||
                (Array.isArray(xLabelListe) && xLabelListe.length === 0)) {
                xLabelListe = rangeMinMax(0, xLabelMax, [0], xLabelDistance).concat(rangeMinMax(0, -xLabelMin, [0], xLabelDistance).map((el) => -el));
            }
            else if (typeof xLabelListe === 'boolean')
                xLabelListe = [];
            for (const x of xLabelListe) {
                let l;
                if (typeof x === 'number') {
                    if (x >= xMin && x <= xMax) {
                        l = latex2d(`${stringNombre(x, precisionLabelX)}`, x * xUnite, ordonneeAxe * yUnite - xLabelEcart + 0.1, 
                    ...
                    if (x.valeur <= xMax && x.valeur >= xMin) {
                        l = latex2d(x.texte, x.valeur * xUnite, ordonneeAxe * yUnite - xLabelEcart + 0.1, 
        ...
        if (labelsYareVisible) {
            if ((typeof yLabelListe === 'boolean' && yLabelListe) ||
                (Array.isArray(yLabelListe) && yLabelListe.length === 0)) {
                yLabelListe = rangeMinMax(0, yLabelMax, [0], yLabelDistance).concat(rangeMinMax(0, -yLabelMin, [0], yLabelDistance).map((el) => -el));
            }
            else if (typeof yLabelListe === 'boolean')
                yLabelListe = [];
            for (const y of yLabelListe) {
                let l;
                if (typeof y === 'number') {
                    if (y >= yMin && y <= yMax) {
                        l = latex2d(`${stringNombre(y, precisionLabelY)}`, abscisseAxe * xUnite - yLabelEcart, y * yUnite + 0.1, 
                    ...
                    if (y.valeur >= yMin && y.valeur <= yMax) {
                        l = latex2d(y.texte, abscisseAxe * xUnite - yLabelEcart, y.valeur * yUnite + 0.1, 
                        /// { letterSize: 'small', opacity: 0.8, color: 'black' },
                        { letterSize: 'scriptsize', opacity: 0.8, color: 'black' });
                        //     l.isVisible = false
                        this.objets.push(l);
        ...
        if (xLegende.length > 0) {
            this.objets.push(texteParPosition(xLegende, xLegendePosition[0], xLegendePosition[1], 0, 'black', 1, 'droite'));
        }
        if (yLegende.length > 0) {
            this.objets.push(texteParPosition(yLegende, yLegendePosition[0], yLegendePosition[1], 0, 'black', 1, 'droite'));
        }
    ...
    addObjet(objet) {
        ...
        if (!(objet instanceof ObjetMathalea2D))
            return;
        (_a = this.objets) === null || _a === void 0 ? void 0 : _a.concat(objet);
    }
 ...
 * @author Rémi Angot
 */
export function repere({ xUnite = 1, yUnite = 1, xMin = -10, xMax = 10, yMin = -10, yMax = 10, axeXisVisible = true, axeYisVisible = true, axesEpaisseur = 1.2, axesCouleur = 'black', axeXStyle = '->', axeYStyle = '->', thickEpaisseur = 1.2, thickHauteur = 0.13, thickCouleur = axesCouleur, xThickDistance = 1, xThickListe = [], xThickMin = xMin + xThickDistance, xThickMax = xMax - xThickDistance, yThickDistance = 1, yThickListe = [], yThickMin = yMin + yThickDistance, yThickMax = yMax - yThickDistance, labelsXareVisible = true, labelsYareVisible = true, xLabelDistance = xThickDistance, xLabelListe = [], xLabelMin = xThickMin, xLabelMax = xThickMax, yLabelDistance = yThickDistance, yLabelListe = [], yLabelMin = yThickMin, yLabelMax = yThickMax, precisionLabelX = 1, precisionLabelY = 1, xLabelEcart = 0.5, yLabelEcart = 0.5, xLegende = '', xLegendePosition = [], yLegende = '', yLegendePosition = [], grille = true, grilleDistance = 1, grilleCouleur = 'black', grilleOpacite = 0.4, grilleEpaisseur = 1, grilleSecondaire = false, grilleSecondaireDistance = 1, grilleSecondaireCouleur = 'gray', grilleSecondaireOpacite = 0.3, grilleSecondaireEpaisseur = 1, grilleX = grille, grilleXListe = [], grilleXDistance = grilleDistance, grilleXMin = xMin, grilleXMax = xMax, grilleXCouleur = grilleCouleur, grilleXOpacite = grilleOpacite, grilleY = grille, grilleYListe = [], grilleYDistance = grilleDistance, grilleYMin = yMin, grilleYMax = yMax, grilleYCouleur = grilleCouleur, grilleYOpacite = grilleOpacite, grilleSecondaireX = grilleSecondaire, grilleSecondaireXListe = [], grilleSecondaireXDistance = grilleSecondaireDistance, grilleSecondaireXMin = xMin, grilleSecondaireXMax = xMax, grilleSecondaireXCouleur = grilleSecondaireCouleur, grilleSecondaireXOpacite = grilleSecondaireOpacite, grilleSecondaireY = grilleSecondaire, grilleSecondaireYListe = [], grilleSecondaireYDistance = grilleSecondaireDistance, grilleSecondaireYMin = yMin, grilleSecondaireYMax = yMax, grilleSecondaireYCouleur = grilleSecondaireCouleur, grilleSecondaireYOpacite = grilleSecondaireOpacite, }) {
    return new Repere({
        xUnite,
        yUnite,
        xMin,
