...
export function patternTikZ(params) {
    const { x0, y0, x1, y1, distanceDesHachures, couleurDesHachures, motif } = params;
    const lignes = [];
    const hauteur = y1 - y0;
    ...
    switch (motif) {
        case 'horizontal lines': {
            for (let y = y0; y <= y1; y += distanceDesHachures) {
                lignes.push(`\\draw[${couleurDesHachures}] (${x0},${y.toFixed(2)}) -- (${x1},${y.toFixed(2)});`);
            }
            break;
        }
        case 'vertical lines': {
            for (let x = x0; x <= x1; x += distanceDesHachures) {
                lignes.push(`\\draw[${couleurDesHachures}] (${x.toFixed(2)},${y0}) -- (${x.toFixed(2)},${y1});`);
            }
            break;
        }
        case 'dots': {
            for (let x = x0; x <= x1; x += distanceDesHachures) {
                for (let y = y0; y <= y1; y += distanceDesHachures) {
                    lignes.push(`\\fill[${couleurDesHachures}] (${x.toFixed(2)},${y.toFixed(2)}) circle (0.05);`);
                }
            }
            break;
        }
        ...
        case 'checkerboard': {
            ...
            for (let y = y0; y < y1; y += side) {
                ...
                for (let x = x0 + (toggle ? 0 : side); x < x1; x += 2 * side) {
                    lignes.push(`\\fill[${couleurDesHachures}] (${x.toFixed(2)},${y.toFixed(2)}) rectangle (${(x + side).toFixed(2)},${(y + side).toFixed(2)});`);
                }
            }
            break;
        }
            ...
            for (let y = y0; y < y1; y += brickHeight) {
                ...
                for (let x = xStart; x < x1; x += brickWidth) {
                    ...
                    if (xRight > x0 && xLeft < x1) {
                        lignes.push(`\\draw(${xLeft.toFixed(2)},${y.toFixed(2)}) rectangle (${xRight.toFixed(2)},${yTop.toFixed(2)});`);
                    }
                }
            }
            break;
        ...
        default: {
            ...
            for (let x = xmin; x <= xmax; x += distanceDesHachures) {
                lignes.push(`\\draw[${couleurDesHachures}] (${x.toFixed(2)},${y0}) -- ++(${hauteur},${hauteur});`);
            }
            break;
        }
    }
 ...
 * @author Jean-Claude Lhote
 */
export function pattern({ motif = 'north east lines', id = '0', distanceDesHachures = 10, epaisseurDesHachures = 1, couleurDesHachures = 'black', couleurDeRemplissage = 'none', opaciteDeRemplissage = 0.5, }) {
    let myPattern = '';
    if (context.isHtml) {
