...
class Grandeur {
    ...
    convertirEn(uniteConversion) {
        ...
        if (uniteConversionParsee.puissanceUnite === this.puissanceUnite &&
        ...
        else if (uniteConversionParsee.uniteDeReference === 'm^3' &&
        ...
        else if (uniteConversionParsee.uniteDeReference === 'L' &&
        ...
        else if (uniteConversionParsee.uniteDeReference === 'm^2' &&
        ...
        else if (uniteConversionParsee.uniteDeReference === 'a' &&
    ...
    estEgal(unite2) {
        if (unite2.uniteDeReference !== this.uniteDeReference)
            return false;
        try {
    ...
    toString(precision = 12) {
        ...
        const nbChiffrePartieDecimale = (_a = String(this.mesure).split('.')[1]) === null || _a === void 0 ? void 0 : _a.length;
        if (nbChiffrePartieDecimale < precision)
            precision = nbChiffrePartieDecimale;
...
// =======================================================
function isVitesseUnit(u) {
    return u === 'm/s' || u === 'km/h';
}
function isDureeUnit(u) {
...
function toMps(value, unit) {
    if (unit === 'm/s')
        return value;
    if (unit === 'km/h')
        return value.div(3.6);
    throw new Error(`Unité de vitesse inconnue: ${unit}`);
}
function fromMps(mps, unit) {
    if (unit === 'm/s')
        return mps;
    if (unit === 'km/h')
        return mps.times(3.6);
    throw new Error(`Unité de vitesse inconnue: ${unit}`);
}
// durée base = secondes
function toSecondsDuree(value, unit) {
    if (unit === 'hhmmss')
        return value; // convention: value already in seconds
    if (unit === 'hdec')
        return value.times(3600);
    if (unit === 'mindec')
...
function fromSecondsDuree(seconds, unit) {
    if (unit === 'hhmmss')
        return seconds; // stockée en secondes
    if (unit === 'hdec')
...
function formatDureeTexHMS(seconds) {
