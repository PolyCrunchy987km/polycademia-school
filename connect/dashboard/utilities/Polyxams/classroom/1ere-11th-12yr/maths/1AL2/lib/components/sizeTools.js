 ...
 */
export const setSizeWithinSvgContainer = (parent) => {
    ...
    do {
        ...
        if (parent.firstElementChild.scrollHeight > originalClientHeight ||
...
function resizeSchemaContainer(schemaContainer, zoom) {
    const originalWidth = schemaContainer.dataset.originalWidth || schemaContainer.scrollWidth;
    const originalHeight = schemaContainer.dataset.originalHeight || schemaContainer.scrollHeight;
...
export function resizeContent(container, zoom) {
    var _a, _b;
    const ZOOM_MIN = 0.2;
    if (!container)
        return;
    // mathalea2d
    const svgContainers = (_a = container.getElementsByClassName('svgContainer')) !== null && _a !== void 0 ? _a : [];
    for (const svgContainer of svgContainers) {
    ...
    // Schémas en boite
    const schemaContainers = (_b = container.getElementsByClassName('SchemaContainer')) !== null && _b !== void 0 ? _b : [];
    for (const schemaContainer of schemaContainers) {
        resizeSchemaContainer(schemaContainer, Math.max(zoom, ZOOM_MIN));
    }
    // Texte
    container.style.fontSize = `${Math.max(zoom, ZOOM_MIN)}rem`;
}
export function updateFigures(svgContainer, zoom) {
    var _a;
    const svgDivs = svgContainer.querySelectorAll('.mathalea2d');
    for (const svgDiv of svgDivs) {
        if (svgDiv instanceof SVGSVGElement) {
            ...
            if (!figure.dataset.viewBoxInitiale) {
                ...
                if (viewBox != null)
                    figure.dataset.viewBoxInitiale = viewBox;
            }
            if (figure.dataset.contentBoundsInitiales === undefined &&
            ...
            const eltsInVariationTables = (_a = svgContainer.getElementsByClassName('divLatex')) !== null && _a !== void 0 ? _a : [];
            for (const elt of eltsInVariationTables) {
                const e = elt;
                if (!e.dataset.top)
                    e.dataset.top = e.style.top.replace('px', '');
                if (!e.dataset.left)
                    e.dataset.left = e.style.left.replace('px', '');
                ...
                e.style.setProperty('left', ((initialLeft - effectiveFrame.xOffset) * zoom).toString() + 'px');
            }
        }
    }
}
function getEffectiveSvgFrame(figure) {
    var _a, _b, _c;
    const width = Number(figure.dataset.widthInitiale);
    const height = Number(figure.dataset.heightInitiale);
    const initialViewBox = (_a = figure.dataset.viewBoxInitiale) === null || _a === void 0 ? void 0 : _a.split(/\s+/).map(Number);
    ...
    if (!initialViewBox || initialViewBox.length !== 4 || !initialBounds) {
        return {
            ...
            height,
            xOffset: 0,
            viewBox: (_b = figure.dataset.viewBoxInitiale) !== null && _b !== void 0 ? _b : null,
        };
    }
    const [viewBoxX, viewBoxY, viewBoxWidth, viewBoxHeight] = initialViewBox;
    const bounds = JSON.parse(initialBounds);
    ...
    const bottomOverflow = bounds.y + bounds.height - (viewBoxY + viewBoxHeight);
    const hasReasonableBounds = Number.isFinite(bounds.x) &&
    ...
    if (!hasReasonableBounds) {
        return {
            ...
            height,
            xOffset: 0,
            viewBox: (_c = figure.dataset.viewBoxInitiale) !== null && _c !== void 0 ? _c : null,
        };
    }
    ...
    const currentRightGap = effectiveX + effectiveWidth - (bounds.x + bounds.width);
 ...
 */
export const resizeTags = (tags, factor = 1) => {
    var _a, _b;
    let widthUnit, heightUnit;
    for (const tag of tags) {
        const widthAttributeExists = tag.hasAttribute('width');
        const heightAttributeExists = tag.hasAttribute('height');
        if (tag.hasAttribute('data-width') === false) {
            ...
            if (widthAttributeExists) {
                ...
                const units = (_a = width.match(/\D/g)) !== null && _a !== void 0 ? _a : [];
                widthUnit = units.join('');
                originalWidth = String(parseFloat(tag.style.width.replace(widthUnit, '')));
            }
            tag.dataset.width = originalWidth !== null && originalWidth !== void 0 ? originalWidth : '50';
        }
        if (!widthAttributeExists &&
        ...
        if (tag.hasAttribute('data-height') === false) {
            ...
            if (heightAttributeExists) {
                ...
                const units = (_b = height.match(/\D/g)) !== null && _b !== void 0 ? _b : [];
                heightUnit = units.join('');
                originalHeight = String(parseFloat(tag.style.height.replace(heightUnit, '')));
            }
            tag.dataset.height = originalHeight !== null && originalHeight !== void 0 ? originalHeight : '30';
        }
        else {
            heightUnit = 'px';
        }
...
export const updateIframeSize = (container, iframe) => {
