...
import { angleOriente, estSecant as estSecantUtil, } from './utilitairesGeometriques';
import { pointSurSegment } from './utilitairesPoint';
/**
 * Rotation d'un point M autour d'un centre O d'un angle en degrés
 */
function rotationPoint(M, O, angleDeg) {
 ...
 */
function similitudePoint(centre, M, angleDeg, rapport) {
    const angleRad = (angleDeg * Math.PI) / 180;
    const cos = Math.cos(angleRad);
    const sin = Math.sin(angleRad);
    const dx = M.x - centre.x;
    const dy = M.y - centre.y;
    return point(centre.x + rapport * (dx * cos - dy * sin), centre.y + rapport * (dx * sin + dy * cos));
}
/**
...
export class Segment extends ObjetMathalea2D {
    constructor(a, b, colorOrX2, styleOrY2, color, styleExtremites) {
        super();
        this.bordures = [0, 0, 0, 0];
        this.typeObjet = 'segment';
        this.tailleExtremites = 4;
        const isPoint = (v) => v instanceof PointAbstrait;
        if (isPoint(a) && isPoint(b)) {
            ...
            this.x2 = b.x;
            this.y2 = b.y;
            this.color = colorToLatexOrHTML(typeof colorOrX2 === 'string' ? colorOrX2 : 'black');
            ...
            if (Number.isNaN(a) ||
                ...
                window.notify('Segment : (attendus : x1, y1, x2, y2 et "couleur") les arguments ne sont pas des nombres valides', { a, b, colorOrX2, styleOrY2 });
                this.x1 = 0;
                this.y1 = 0;
                this.x2 = 0;
                this.y2 = 0;
            ...
            // Cas invalide
            window.notify('Segment : utilisez (A, B, color?, style?) ou (x1, y1, x2, y2, color?, style?)', { a, b, colorOrX2, styleOrY2 });
            this.x1 = 0;
            this.y1 = 0;
            this.x2 = 0;
            this.y2 = 0;
        ...
        this.angleAvecHorizontale =
            this.longueur < 1e-8
                ? 0
                : angleOriente(point(this.x1 + 1, this.y1), this.extremite1, this.extremite2, 5);
        if (this.longueur < 1e-8) {
            window.notify("Création d'un segment de longueur nulle, les opérations géométriques peuvent être erronées", { x1: this.x1, y1: this.y1, x2: this.x2, y2: this.y2 });
        }
    }
    svg(coeff) {
        if (this.epaisseur !== 1) {
        ...
        code += `\n\t<line x1="${A.xSVG(coeff)}" y1="${A.ySVG(coeff)}" x2="${B.xSVG(coeff)}" y2="${B.ySVG(coeff)}" stroke="${this.color[0]}" ${this.style} />`;
    ...
    tikz() {
        ...
        if (this.styleExtremites.length > 0) {
            ...
            if (this.styleExtremites.length === 3) {
                ...
                if (this.styleExtremites[0] in map)
                    left = map[this.styleExtremites[0]];
                if (this.styleExtremites[1] in map)
                    right = map[this.styleExtremites[1]];
            }
            else if (this.styleExtremites.length === 1) {
                if (this.styleExtremites[0] in map)
                    right = map[this.styleExtremites[0]];
            }
    ...
    tikzml(amp) {
        ...
        if (this.styleExtremites.length > 1) {
            if (this.styleExtremites.includes('[') ||
    ...
    codeExtremitesSVG(coeff) {
        ...
        if (this.styleExtremites.length > 1) {
            ...
            if (fin === '|') {
                ...
                code += `<line x1="${B1.xSVG(coeff)}" y1="${B1.ySVG(coeff)}" x2="${B2.xSVG(coeff)}" y2="${B2.ySVG(coeff)}" stroke="${this.color[0]}" stroke-width="${this.epaisseur}" />`;
            }
            if (fin === '>') {
                ...
                code += `<line x1="${B.xSVG(coeff)}" y1="${B.ySVG(coeff)}" x2="${B1.xSVG(coeff)}" y2="${B1.ySVG(coeff)}" stroke="${this.color[0]}" stroke-width="${this.epaisseur}" />`;
                code += `\n\t<line x1="${B.xSVG(coeff)}" y1="${B.ySVG(coeff)}" x2="${B2.xSVG(coeff)}" y2="${B2.ySVG(coeff)}" stroke="${this.color[0]}" stroke-width="${this.epaisseur}" />`;
            }
            if (fin === '<') {
            ...
            if (debut === '<') {
                ...
                code += `<line x1="${A.xSVG(coeff)}" y1="${A.ySVG(coeff)}" x2="${A1.xSVG(coeff)}" y2="${A1.ySVG(coeff)}" stroke="${this.color[0]}" stroke-width="${this.epaisseur}" />`;
                code += `\n\t<line x1="${A.xSVG(coeff)}" y1="${A.ySVG(coeff)}" x2="${A2.xSVG(coeff)}" y2="${A2.ySVG(coeff)}" stroke="${this.color[0]}" stroke-width="${this.epaisseur}" />`;
            }
            if (debut === '>') {
            ...
            if (debut === '|') {
                ...
                code += `<line x1="${A1.xSVG(coeff)}" y1="${A1.ySVG(coeff)}" x2="${A2.xSVG(coeff)}" y2="${A2.ySVG(coeff)}" stroke="${this.color[0]}" stroke-width="${this.epaisseur}" />`;
            }
        }
        return code;
    }
...
export function segment(a, b, colorOrX2, styleOrY2, color, styleExtremites) {
    return new Segment(a, b, colorOrX2, styleOrY2, color, styleExtremites);
}
/**
...
export function segmentAvecExtremites(...args) {
    const s = typeof args[0] === 'number' && typeof args[1] === 'number'
        ? segment(args[0], args[1], args[2], args[3], args[4], args[5])
        : segment(args[0], args[1], args[2], args[3]);
    s.styleExtremites = '|-|';
    return s;
}
