...
import { isIExercice, } from '../lib/types';
...
function testIfLoaded(values, valuetoSearch, valueToPut) {
    let search = false;
...
function estimerLongueurLatex(texte, options = {}) {
...
class Latex {
    ...
    addExercices(exercices) {
        this.exercices.push(...exercices);
    }
    getContentsForAVersion(latexFileInfos, indiceVersion = 1) {
        var _a, _b, _c, _d, _e;
        if (latexFileInfos.style === 'ProfMaquette') {
            return {
                content: this.getContentForAVersionProfMaquette(indiceVersion, latexFileInfos),
                contentCorr: '',
            };
        }
            ...
            for (const exercice of this.exercices) {
                if (exercice != null) {
                    if (exercice.typeExercice === 'statique') {
                        ...
                        const questionLiee = Array.from({ length: (_a = exercice.listeQuestions) === null || _a === void 0 ? void 0 : _a.length }, () => ({
                            compteurQuestionsLiees: 0,
                            dejaLiee: false,
                        }));
                        for (let i = 0; i < exercice.listeQuestions.length; i++) {
                            // Enoncé de la question
                            const enonce = exercice.listeCanEnonces != null &&
                            ...
                            if (exercice.listeCanLiees != null &&
            ...
            for (const exercice of this.exercices) {
                if (exercice.typeExercice === 'statique') {
                    if (exercice.content === '') {
                        ...
                        content += testIfLoaded([(_b = exercice.content) !== null && _b !== void 0 ? _b : ''], '\\anote{', '\n\\resetcustomnotes');
                        content += exercice.content;
                        content += testIfLoaded([(_c = exercice.content) !== null && _c !== void 0 ? _c : ''], '\\anote{', '\n\\printcustomnotes');
                        content += '\n\\end{EXO}\n';
                        contentCorr += '\n\\begin{EXO}{}{}\n';
                        contentCorr += testIfLoaded([(_d = exercice.contentCorr) !== null && _d !== void 0 ? _d : ''], '\\anote{', '\n\\resetcustomnotes');
                        contentCorr += exercice.contentCorr;
                        contentCorr += testIfLoaded([(_e = exercice.contentCorr) !== null && _e !== void 0 ? _e : ''], '\\anote{', '\n\\printcustomnotes');
                        contentCorr += '\n\\end{EXO}\n';
                    }
                }
                else if (isIExercice(exercice)) {
                    ...
                    contentCorr += buildContent(exercice.listeCorrections, exercice.spacingCorr, Boolean(exercice.listeAvecNumerotation), Number(exercice.nbColsCorr));
                    contentCorr += testIfLoaded(exercice.listeCorrections, '\\anote{', '\n\\printcustomnotes');
                    contentCorr += '\n\\end{EXO}\n';
                    content += `\n% @see : ${getUrlFromExercice(exercice)}`;
                    ...
                    content += buildContent(exercice.listeQuestions, exercice.spacing, Boolean(exercice.listeAvecNumerotation), Number(exercice.nbCols));
    ...
    loadExercicesWithVersion(indiceVersion = 1) {
        var _a, _b, _c;
        for (const exercice of this.exercices) {
            if (exercice.typeExercice === 'statique') {
                const serie = (_a = exercice === null || exercice === void 0 ? void 0 : exercice.examen) === null || _a === void 0 ? void 0 : _a.toLowerCase();
                if (serie === 'crpe' && indiceVersion === 1) {
                    exercice.content = (_b = exercice.content) === null || _b === void 0 ? void 0 : _b.replaceAll('{Images/', '{');
                    exercice.contentCorr = (_c = exercice.contentCorr) === null || _c === void 0 ? void 0 : _c.replaceAll('{Images/', '{');
                }
                continue;
            }
    ...
    getContentForAVersionProfMaquette(indiceVersion = 1, latexFileInfos) {
        var _a;
        this.loadExercicesWithVersion(indiceVersion);
        let content = '';
        const groups = decodeExosGrouping((_a = latexFileInfos.exosGrouping) !== null && _a !== void 0 ? _a : '', this.exercices.length);
        for (let groupIndex = 0; groupIndex < groups.length; groupIndex++) {
            ...
            for (let i = 0; i < groupNbrs.length; i++) {
                ...
                if (exercice.typeExercice === 'statique') {
                    ...
                    content += this.generateStaticExerciseContent(exercice, latexFileInfos, indiceVersion);
                }
                else if (isIExercice(exercice)) {
                    ...
                    const prefix = [exercice.introduction, exercice.consigne]
                        .filter(Boolean)
                        .join('\n');
                    const questionsAvecIntro = exercice.listeQuestions.map((q) => `${prefix}\n${q}`);
            ...
            if (exoGroups.listeQuestions.length === 0)
                continue;
            const confExo = latexFileInfos.exos && latexFileInfos.exos[groupNbrs[0]]
                ? latexFileInfos.exos[groupNbrs[0]]
                : {};
            content +=
                contentlocal +
                    this.generateExerciseContent(latexFileInfos, exoGroups, indiceVersion, confExo);
        }
        for (let k = 0; k < this.exercices.length; k++) {
            if (findExoPosition(groups, k))
                continue; // cet exercice est déjà dans un groupe, on l'ignore ici
            const exercice = this.exercices[k];
            content += `\n% @see : ${getUrlFromExercice(exercice, indiceVersion)}`;
            const confExo = latexFileInfos.exos && latexFileInfos.exos[k]
            ...
            if (exercice.typeExercice === 'statique') {
                ...
                content += this.generateExerciseContent(latexFileInfos, exercice, indiceVersion, confExo);
            }
        }
        return content;
    }
    generateStaticExerciseContent(exercice, latexFileInfos, indiceVersion) {
        let content = '';
        if (exercice.content === '') {
            content += "% Cet exercice n'est pas disponible au format LaTeX";
        }
            ...
            if (latexFileInfos.qrcodeOption === 'AvecQrcode') {
                content += `\n\\begin{exercice}[${latexFileInfos.titleOption === 'AvecTitre'
    ...
    generateExerciseContent(latexFileInfos, exercice, indiceVersion, confExo) {
        let content = '\n\\needspace{10\\baselineskip}';
        if (latexFileInfos.qrcodeOption === 'AvecQrcode') {
            content += `\n\\begin{exercice}[${latexFileInfos.titleOption === 'AvecTitre'
        ...
        if (latexFileInfos.qrcodeOption === 'AvecQrcode') {
            const phrases = exercice.listeQuestions.length > 0
                ? exercice.listeQuestions[0].split(/\\\\|\r?\n/)
                : [];
            const firstQuestion = phrases.length > 0 ? estimerLongueurLatex(phrases[0]) : '';
            const secondQuestion = phrases.length > 1 ? estimerLongueurLatex(phrases[1]) : '';
            if (latexFileInfos.qrcodeOption === 'AvecQrcode' &&
        ...
        content += buildContent(exercice.listeQuestions, exercice.spacing, Boolean(exercice.listeAvecNumerotation), confExo.cols ? confExo.cols : Number(exercice.nbCols), confExo);
        content += testIfLoaded([...exercice.listeQuestions, exercice.consigne, exercice.introduction], '\\anote{', '\n\\printcustomnotes');
        content += '\n\\end{exercice}\n';
        content += '\n\\begin{Solution}';
        content += testIfLoaded([...exercice.listeCorrections], '\\anote{', '\n\\resetcustomnotes');
        content += buildContent(exercice.listeCorrections, exercice.spacingCorr, Boolean(exercice.listeAvecNumerotation), confExo.cols_corr ? confExo.cols_corr : Number(exercice.nbColsCorr), confExo.labels ? { labels: confExo.labels } : {});
        content += testIfLoaded([...exercice.listeCorrections], '\\anote{', '\n\\printcustomnotes');
        content += '\n\\end{Solution}\n';
        return content;
    }
    async getContents(latexFileInfos) {
        var _a, _b, _c, _d, _e;
        const contents = {
            preamble: '',
            intro: '',
        ...
        if (latexFileInfos.style === 'ProfMaquette' ||
            ...
            if (latexFileInfos.style === 'ProfMaquette') {
                for (let i = 1; i < latexFileInfos.nbVersions + 1; i++) {
                    if ((_a = latexFileInfos.signal) === null || _a === void 0 ? void 0 : _a.aborted) {
                        throw new DOMException('Aborted in getContents of Latex.ts', 'AbortError');
                    }
                    const contentVersion = this.getContentForAVersionProfMaquette(i, latexFileInfos);
                ...
                for (let i = 1; i < latexFileInfos.nbVersions + 1; i++) {
                    if ((_b = latexFileInfos.signal) === null || _b === void 0 ? void 0 : _b.aborted) {
                        throw new DOMException('Aborted2 in getContents of Latex.ts', 'AbortError');
                    }
                    latexFileInfos.qrcodeOption = 'AvecQrcode';
                    const contentVersion = this.getContentForAVersionProfMaquette(i, latexFileInfos);
            ...
            if ((_c = latexFileInfos.signal) === null || _c === void 0 ? void 0 : _c.aborted) {
                throw new DOMException('Aborted3 in getContents of Latex.ts', 'AbortError');
            }
            this.loadPreambuleFromContents(contents, latexFileInfos);
            if (latexFileInfos.modele !== undefined &&
            ...
            for (let i = 1; i < latexFileInfos.nbVersions + 1; i++) {
                if ((_d = latexFileInfos.signal) === null || _d === void 0 ? void 0 : _d.aborted) {
                    throw new DOMException('Aborted in getContents of Latex.ts', 'AbortError');
                }
            ...
            if ((_e = latexFileInfos.signal) === null || _e === void 0 ? void 0 : _e.aborted) {
                throw new DOMException('Aborted in getContents of Latex.ts', 'AbortError');
            }
            if (latexFileInfos.style === 'Can') {
    ...
    loadPreambuleFromContents(contents, latexFileInfos) {
        const currentUrl = this.getURL();
        contents.preamble = `% @see : ${currentUrl.href.replaceAll('%', '\\%')}`;
        contents.preamble += '\n\\documentclass[a4paper,11pt,fleqn]{article}';
...
\\hypersetup{
    ...
    async getFile(latexFileInfos) {
        const contents = await this.getContents(latexFileInfos);
        const preamble = contents === null || contents === void 0 ? void 0 : contents.preamble;
        const intro = contents === null || contents === void 0 ? void 0 : contents.intro;
        const content = contents === null || contents === void 0 ? void 0 : contents.content;
        const contentCorr = contents === null || contents === void 0 ? void 0 : contents.contentCorr;
    ...
    getContentLatex() {
        ...
        const packageFiltered = packLatex.filter((value, index, array) => array.indexOf(value) === index);
        const [latexCmds, latexPackages] = packageFiltered.reduce((result, element) => {
...
function writeIntroduction(introduction = '') {
    let content = '';
...
function buildContent(questions, spacing = 1, numbersNeeded, nbCols = 1, confExo = {}) {
    let content = '';
 ...
 * @author sylvain
 */
export function buildImagesUrlsList(exosContentList, picsNames) {
    const imagesFilesUrls = [];
    exosContentList.forEach((exo, i) => {
        var _a;
        if (picsNames[i].length !== 0) {
            const year = exo.year;
            const serie = (_a = exo === null || exo === void 0 ? void 0 : exo.serie) === null || _a === void 0 ? void 0 : _a.toLowerCase();
 ...
 * @author sylvain
 */
export function getExosContentList(exercices) {
    var _a;
    const exosContentList = [];
    for (const exo of exercices) {
        let data = {};
        if (exo.typeExercice === undefined) {
            Object.assign(data, {}, { content: (_a = exo.contenu) !== null && _a !== void 0 ? _a : '' });
        }
...
export function getPicsNames(exosContentList) {
    const picsList = [];
    const picsNames = [];
...
export function makeImageFilesUrls(exercices) {
 ...
 */
export function format(text, AvecLesDoublesEspaces = true) {
    if (text === undefined)
        return '';
    const lang = getLang();
...
function getUrlFromExercice(ex, version = 1) {
    ...
    if (isIExercice(ex)) {
        ...
        if (ex.sup !== undefined)
            url.searchParams.append('s', ex.sup);
        if (ex.sup2 !== undefined)
            url.searchParams.append('s2', ex.sup2);
        if (ex.sup3 !== undefined)
            url.searchParams.append('s3', ex.sup3);
        if (ex.sup4 !== undefined)
            url.searchParams.append('s4', ex.sup4);
        if (ex.sup5 !== undefined)
            url.searchParams.append('s5', ex.sup5);
        if (ex.seed !== undefined)
            url.searchParams.append('alea', version > 1 ? ex.seed + version.toString : ex.seed);
        if (ex.interactif)
            url.searchParams.append('i', '1');
...
function addPackages(latexFileInfos, contents) {
