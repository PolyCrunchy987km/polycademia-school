...
import { isPointsAbstraits, pointAbstrait, PointAbstrait, } from './PointAbstrait';
...
function aireTriangle(p) {
    if (!(p === null || p === void 0 ? void 0 : p.listePoints) || p.listePoints.length !== 3)
        return false;
...
export class Polygone extends ObjetMathalea2D {
    constructor(...points) {
        ...
        if (Array.isArray(points[0])) {
            ...
            this.listePoints = points[0].map((el) => pointAbstrait(el.x, el.y, el.nom, el.positionLabel));
            if (points[1]) {
        ...
        for (const unPoint of this.listePoints) {
            if (unPoint.typeObjet !== 'point')
                window.notify('Polygone : argument invalide', Object.assign({}, points));
        ...
        if (this._triangulation.length === 0) {
            ...
            for (let i = 0; i < trianglesIndices.length; i += 3) {
                this._triangulation.push([
                    pointAbstrait(this.flat[trianglesIndices[i] * 2], this.flat[trianglesIndices[i] * 2 + 1]),
                    pointAbstrait(this.flat[trianglesIndices[i + 1] * 2], this.flat[trianglesIndices[i + 1] * 2 + 1]),
                    pointAbstrait(this.flat[trianglesIndices[i + 2] * 2], this.flat[trianglesIndices[i + 2] * 2 + 1]),
                ]);
            }
        }
...
export class PolygoneATrous extends ObjetMathalea2D {
    constructor({ data = [], holes = [], noms = '', color = 'black', couleurDeRemplissage = 'blue', couleurDeFond = 'white', }) {
        super();
    ...
    get triangulation() {
        if (this._triangulation === null) {
            ...
            for (let i = 0, triangle; i < this.triangles.length; i += 3) {
                triangle = polygone([
                    pointAbstrait(this.data[this.triangles[i] * 2], this.data[this.triangles[i] * 2 + 1]),
                    pointAbstrait(this.data[this.triangles[i + 1] * 2], this.data[this.triangles[i + 1] * 2 + 1]),
                    pointAbstrait(this.data[this.triangles[i + 2] * 2], this.data[this.triangles[i + 2] * 2 + 1]),
                ]);
...
export class NommePolygone extends ObjetMathalea2D {
    constructor(p, nom = '', k = 0.5, color = 'black', size = 1) {
        super();
        this.poly = p;
        this.dist = k;
        this.objets = [];
        ...
        for (const pt of p.listePoints) {
            const P = pointSurSegment(G, pt, longueur(G, pt) +
...
export function nommePolygone(p, nom = '', k = 0.5, color = 'black', size = 1) {
