...
import { decompositionFacteursPremiers, obtenirListeFacteursPremiers, ppcm, } from '../lib/outils/primalite';
import { texNombre } from '../lib/outils/texNombre';
// Versions locales (scope module) pour éviter la dépendance circulaire avec ecritures.ts
function ecritureAlgebrique(n) {
    return n >= 0 ? '+' + texNombre(n, 7) : texNombre(n, 7);
}
function ecritureParentheseSiNegatif(n) {
    return n >= 0 ? texNombre(n, 8) : `(${texNombre(n, 8)})`;
}
function signeMoinsEnEvidence(r, precision = 0) {
    if (typeof r !== 'number') {
        window.notify("signeMoinsEnEvidence() appelé avec autre chose qu'un nombre.", { argument: r });
    }
    if (r < 0) {
...
function normalizeFraction(n, d) {
    ...
    if (d == null) {
        ...
        if (Number.isInteger(n) && Number.isInteger(d)) {
            ...
            const pow = Math.min(5, Math.max(nNbChiffresDecimaux, dNbChiffresDecimaux));
            if (pow > 0) {
    ...
    if (isNaN(num) || isNaN(den)) {
        window.notify('FractionEtendue : valeurs impossibles à convertir en fraction', { n, d });
        return [NaN, NaN];
    }
    return [num, den];
}
...
class FractionEtendue {
    ...
    constructor(n, d) {
        ...
        if (isNaN(num) || isNaN(den))
            throw Error(`Fraction Etendue les données ne permettent pas de définir une fraction : n=${n}, d=${d}`);
        const pgcd = Math.abs(num) === 1 && Math.abs(den) === 1
            ? 1
         ...
         * @type {number}
         */
        let sign; // Au cas où quelqu'un le demande en anglais
    ...
    estUneSimplification(f2) {
        if (f2.num == null || f2.den == null)
            throw Error(`estUneSimplification(f2) : f2 n'est pas une FractionEtendue ! f2=${JSON.stringify(f2)}`);
    ...
    sommeFraction(f2) {
        if (f2 instanceof FractionEtendue) {
            if (this.den === f2.den) {
                ...
                return new FractionEtendue(this.num * round(lcm(this.den, f2.den) / this.den) +
                    f2.num * round(lcm(this.den, f2.den) / f2.den), lcm(this.den, f2.den));
            }
        }
        else {
    ...
    texProduitFraction(f2, simplification = 'none') {
        if (this.estEntiere) {
            return `${this.texFraction}\\times ${f2.texFraction}=\\dfrac{${`${this.simplifie().num}\\times ${ecritureParentheseSiNegatif(f2.num)}`}}{${ecritureParentheseSiNegatif(f2.den)}}${simplification === 'none' || this.produitFraction(f2).estIrreductible
                ? '=\\dfrac{' + this.num * f2.num + '}{' + this.den * f2.den + '}'
                : this.produitFraction(f2).texSimplificationAvecEtapes(simplification)}`;
        }
        else {
            return `${this.texFraction}\\times ${f2.texFraction}=\\dfrac{${this.num}\\times ${ecritureParentheseSiNegatif(f2.num)}}{${this.den} \\times ${ecritureParentheseSiNegatif(f2.den)}}${simplification === 'none' || this.produitFraction(f2).estIrreductible
                    ...
                    : this.den !== f2.num && f2.den !== this.num
                        ...
                        : `${this.den === f2.num
                            ? '=\\dfrac{' +
                                ...
                                f2.den +
                                '}' +
                                new FractionEtendue(this.num, f2.den).texSimplificationAvecEtapes(simplification)
                            : '=\\dfrac{' +
                                f2.num +
                                '}{' +
                                this.den +
                                '}' +
                                new FractionEtendue(f2.num, this.den).texSimplificationAvecEtapes(simplification)}`}`}`;
        }
    }
    /**
     * @param {FractionEtendue} f2 la  FractionMathjs  qui multiplie.
    ...
    texDiviseFraction(f2, simplification = 'none', symbole = '/') {
        ...
        if (this.estEntiere) {
            if (f2.inverse().estEntiere && f2.num === 1) {
                ...
                if (symbole === '/') {
                    return `\\dfrac{${space2 + this.texFraction + space2}}{${(f2.estEntiere ? space2 : space) + f2.texFraction + (f2.estEntiere ? space2 : space)}}=${this.texFractionSimplifiee}\\times ${f2.inverse().texFraction}=\\dfrac{${this.texFractionSimplifiee} \\times ${ecritureParentheseSiNegatif(f2.den)}}{${ecritureParentheseSiNegatif(f2.num)}}${simplification === 'none' || this.diviseFraction(f2).estIrreductible
                        ? `=\\dfrac{${this.simplifie().num * f2.den}}{${f2.num}}`
                        : `=\\dfrac{${decompositionFacteursPremiers(this.num)}\\times ${decompositionFacteursPremiers(f2.den)}}{${decompositionFacteursPremiers(f2.num)}}${this.diviseFraction(f2).texSimplificationAvecEtapes(simplification)}`}`;
                }
                else {
                    return `${this.texFraction}\\div${f2.texFraction}=${this.texFractionSimplifiee}\\times ${f2.inverse().texFraction}=\\dfrac{${this.texFractionSimplifiee + '\\times ' + ecritureParentheseSiNegatif(f2.den)}}{${ecritureParentheseSiNegatif(f2.num)}}${simplification === 'none' || this.diviseFraction(f2).estIrreductible
                        ? `=\\dfrac{${this.simplifie().num * f2.den}}{${f2.num}}`
      ...
      ${simplification === 'none' || this.diviseFraction(f2).estIrreductible
                    ...
                    return `${this.texFraction}\\div${f2.texFraction}=${this.texFraction}\\times ${f2.inverse().texFraction}=\\dfrac{${this.num + '\\times ' + f2.inverse().texFraction}}{${this.den}}${simplification === 'none' || this.diviseFraction(f2).estIrreductible
                        ? `=\\dfrac{${this.num * f2.den}}{${this.den * f2.num}}`
                ...
                if (symbole === '/') {
                    return `\\dfrac{${space + this.texFraction + space}}{${(f2.estEntiere ? space2 : space) + f2.texFraction + (f2.estEntiere ? space2 : space)}}=${this.texFraction}\\times ${f2.inverse().texFraction}=\\dfrac{${this.num + '\\times ' + ecritureParentheseSiNegatif(f2.den)}}{${this.den + '\\times ' + ecritureParentheseSiNegatif(f2.num)}}${simplification === 'none' || this.diviseFraction(f2).estIrreductible
                    ...
                    return `${this.texFraction}\\div${f2.texFraction}=${this.texFraction}\\times ${f2.inverse().texFraction}=\\dfrac{${this.num + '\\times ' + f2.den}}{${this.den + '\\times ' + f2.num}}${simplification === 'none' || this.diviseFraction(f2).estIrreductible
                        ? `=\\dfrac{${this.num * f2.den}}{${this.den * f2.num}}`
                        : `=\\dfrac{${decompositionFacteursPremiers(this.num)}\\times ${decompositionFacteursPremiers(f2.den)}}{${decompositionFacteursPremiers(this.den)}\\times ${decompositionFacteursPremiers(f2.num)}}${this.diviseFraction(f2).texSimplificationAvecEtapes(simplification)}`}`;
                }
            }
    ...
    diviseFraction(f2) {
        if (!(f2 instanceof FractionEtendue)) {
            ...
            if (typeof f2 === 'number')
                return this.multiplieEntier(1 / f2);
            else {
            ...
            if (factorisation) {
                ...
                if (pgcd !== 1) {
                    ...
                    if (Math.abs(den / pgcd) !== 1)
                        ...
                        redactionFinale = `${signe}${Math.abs(num / pgcd)}`;
                    if (couleurFinale !== '')
    ...
    texRacineCarree(detaillee = false) {
        if (this.num === 0) {
            ...
            if (this.estParfaite) {
                ...
                if (rac)
                    return rac.texFraction;
                else
                    return `\\sqrt{${this.texFraction}}`;
            }
            if (this.signe === -1)
            ...
            if (arrondi(factoNum[1] / factoDen[1], 6) === 1) {
                ...
                if (k.numIrred === 1 && k.denIrred !== 1) {
                    if (r.denIrred === 1) {
                        ...
                        return ((k.valeurDecimale === 1 ? etape : etape + k.texFraction) +
                            `\\sqrt{${r.toLatex()}}`);
                    }
                }
                else {
