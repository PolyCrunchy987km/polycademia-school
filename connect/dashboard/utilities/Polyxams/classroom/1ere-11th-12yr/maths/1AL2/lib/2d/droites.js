 ...
 */
export function labelOnLine(droite, nom, { preferedPosition = 'auto', usedPosition = [], letterSize = 'footnotesize', color = 'red', backgroundColor = 'white', } = {}) {
    const debug = false;
    const largeur = 30;
    const hauteur = 20;
    ...
    if (nom !== '') {
        if (egal(droite.b, 0, 0.05)) {
            ...
            if (y0 > context.fenetreMathalea2d[1] &&
                ...
                oneUsedPosition = 'left';
                if (ordNom < context.fenetreMathalea2d[1] + 1 ||
                    ordNom > context.fenetreMathalea2d[3] - 1) {
                    if (debug)
                        console.info('probl:nom:' +
                            nom +
                            ':position:' +
                            oneUsedPosition +
            ...
            if (y1 > context.fenetreMathalea2d[1] &&
            ...
            if (x0 > context.fenetreMathalea2d[0] &&
                ...
                oneUsedPosition = 'below';
                if (absNom < context.fenetreMathalea2d[0] + 1 ||
                    absNom > context.fenetreMathalea2d[2] - 1) {
                    if (debug)
                        console.info('problème:nom:' +
                            nom +
                            ':position:' +
                            oneUsedPosition +
            ...
            if (x1 > context.fenetreMathalea2d[0] &&
        ...
        for (let i = 0; i < positions.length; i++) {
            if (positions[i].position === 'middle')
                continue;
            const coli = [];
            for (let j = 0; j < usedPosition.length; j++) {
                const label = usedPosition[j];
                const dis = Math.sqrt((label.x - positions[i].label.x) ** 2 +
                    (label.y - positions[i].label.y) ** 2) * context.pixelsParCm;
                ...
                const colision = XYlabel[0] < XYlabel2[1] &&
                ...
                let colision2 = true;
                if (dis > r0 + r1 || dis < Math.abs(r0 - r1))
                    colision2 = false;
                coli[j] = [dis, colision];
                if (debug)
                    console.info('coli:nom:' +
        ...
        for (let i = 0; i < positions.length && !found[0]; i++) {
            ...
            if (positions[i].position === preferedPosition) {
                found[0] = true;
                for (let j = 0; j < usedPosition.length; j++) {
                    if (positions[i].colision[j][1])
                        found[0] = false;
                    if (debug)
                        console.info('1er:nom:' +
                            nom +
                            ':position:' +
                            positions[i].position +
                            ':i:' +
 ...
 */
export function droiteAvecNomLatex(d, nom, color = 'black') {
    // nom est un latexParCoordonnees
    d.color = colorToLatexOrHTML(color !== null && color !== void 0 ? color : 'black');
    let absNom, ordNom;
    d.epaisseur = 1;
    d.opacite = 1;
    if (egal(d.b, 0, 0.05)) {
...
export class Droite extends ObjetMathalea2D {
    constructor(arg1, arg2, arg3, arg4, arg5) {
        ...
        this.angleAvecHorizontale = angleOriente(pointAbstrait(1, 0), pointAbstrait(0, 0), pointAbstrait(this.directeur.x, this.directeur.y));
        this.bordures = [
        ...
        if (this.nom !== '') {
            ...
            // } else {
            this.leNom = texteParPosition(this.nom, absNom, ordNom, 0, this.stringColor, 1, 'milieu', true);
            //  }
        }
        if (this.nom.includes('$')) {
    ...
    svg(coeff) {
        ...
        if (this.nom === '') {
            return `<line x1="${A1.xSVG(coeff)}" y1="${A1.ySVG(coeff)}" x2="${B1.xSVG(coeff)}" y2="${B1.ySVG(coeff)}" stroke="${this.color[0]}" ${this.style} id ="${this.id}" />`;
        }
        else {
            return (`<line x1="${A1.xSVG(coeff)}" y1="${A1.ySVG(coeff)}" x2="${B1.xSVG(coeff)}" y2="${B1.ySVG(coeff)}" stroke="${this.color[0]}" ${this.style} id ="${this.id}" />` +
                (this.leNom != null ? ((_b = (_a = this.leNom) === null || _a === void 0 ? void 0 : _a.svg(coeff)) !== null && _b !== void 0 ? _b : '') : ''));
        }
    }
    tikz(axisYMin, axisYMax, axisXMin, axisXMax) {
        ...
        const optionsDraw = tableauOptions.length > 0 ? '[' + tableauOptions.join(',') + ']' : '';
        const shouldUsePgfplots = this.usePgfplots &&
            axisYMin !== undefined &&
            axisYMax !== undefined &&
            axisXMin !== undefined &&
            axisXMax !== undefined;
        const formatNombre = (val) => Number.isFinite(val) ? Number(val.toFixed(4)).toString() : '0';
        const formatSlope = (val) => {
            const formatted = Number(val.toFixed(6));
            if (Math.abs(formatted) < 1e-9)
                return '0';
        ...
        if (shouldUsePgfplots) {
            ...
            const domainMin = (_b = (_a = this.pgfplotsDomain) === null || _a === void 0 ? void 0 : _a.xmin) !== null && _b !== void 0 ? _b : axisXMin;
            const domainMax = (_d = (_c = this.pgfplotsDomain) === null || _c === void 0 ? void 0 : _c.xmax) !== null && _d !== void 0 ? _d : axisXMax;
            const rangeMin = (_f = (_e = this.pgfplotsDomain) === null || _e === void 0 ? void 0 : _e.ymin) !== null && _f !== void 0 ? _f : axisYMin;
            const rangeMax = (_h = (_g = this.pgfplotsDomain) === null || _g === void 0 ? void 0 : _g.ymax) !== null && _h !== void 0 ? _h : axisYMax;
            const optionsList = [...pgfplotsOptions];
            const vertical = egal(this.b, 0, 0.0001);
            let code = '';
            if (vertical) {
                const xValue = formatNombre(this.x1);
                const coordYMin = formatNombre(rangeMin !== null && rangeMin !== void 0 ? rangeMin : axisYMin);
                const coordYMax = formatNombre(rangeMax !== null && rangeMax !== void 0 ? rangeMax : axisYMax);
                ...
                if (domainMin !== undefined && domainMax !== undefined) {
                    optionsList.push(`domain=${formatNombre(domainMin)}:${formatNombre(domainMax)}`);
                }
                if (rangeMin !== undefined && rangeMax !== undefined) {
                    optionsList.push(`restrict y to domain=${formatNombre(rangeMin)}:${formatNombre(rangeMax)}`);
                }
                const options = optionsList.length > 0 ? '[' + optionsList.join(',') + ']' : '';
        ...
        if (this.nom !== '') {
            return (`\\draw${optionsDraw} (${A1.x},${A1.y})--(${B1.x},${B1.y});` +
                (this.leNom != null ? ((_k = (_j = this.leNom) === null || _j === void 0 ? void 0 : _j.tikz()) !== null && _k !== void 0 ? _k : '') : ''));
        }
        else {
            return `\\draw${optionsDraw} (${A1.x},${A1.y})--(${B1.x},${B1.y});`;
        }
...
export function droite(arg1, arg2, arg3, arg4, arg5) {
    if (arguments.length === 2)
        return new Droite(arg1, arg2);
    if (arguments.length === 3)
        return new Droite(arg1, arg2, arg3);
    if (arguments.length === 4)
        return new Droite(arg1, arg2, arg3, arg4);
...
export function dessousDessus(d, A, tolerance = 0.0001) {
    if (egal(d.a * A.x + d.b * A.y + d.c, 0, tolerance))
        return 'sur';
    if (egal(d.b, 0)) {
        if (A.x < -d.c / d.a)
            return 'gauche';
