 ...
 */
const longueur = (A, B) => Math.sqrt((B.x - A.x) ** 2 + (B.y - A.y) ** 2);
/**
...
export function pointSurDroite(d, x, nom = '', positionLabel = 'above') {
    ...
    if (d.b === 0)
        return pointAbstrait(-d.c / d.a, x, nom, positionLabel);
    else if (d.a === 0)
        return pointAbstrait(x, -d.c / d.b, nom, positionLabel);
    else
        return pointAbstrait(x, (-d.c - d.a * x) / d.b, nom, positionLabel);
}
/**
 ...
 * @author Jean-Claude Lhote
 */
export function pointAdistance(A, d = 1, angle = 0, nom = '', positionLabel = 'above') {
    let leNom = '';
    let lAngle = 0;
    ...
    if (typeof angle === 'string') {
        ...
        lAngle = angle !== null && angle !== void 0 ? angle : randint(0, 360);
        if (nom != null)
            leNom = nom;
        if (positionLabel != null)
            lePositionLabel = positionLabel;
    }
...
function pointIntersectionNonTrouveEntre(objet1, objet2, valeurParDefaut) {
    window.notify(`${JSON.stringify(objet1)} et ${JSON.stringify(objet2)} ne se coupent pas. Impossible de trouver leur intersection.`, { objet1, objet2 });
